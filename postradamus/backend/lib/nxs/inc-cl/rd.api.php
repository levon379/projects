...
if (!class_exists("nxs_class_SNAP_RD")) {
    class nxs_class_SNAP_RD
    {
        ...
        function doPostToNT($options, $message)
        {
            ...
            //## Format Post
            if (!empty($message['pTitle'])) $title = $message['pTitle']; else $title = nxs_doFormatMsg($options['rdTitleFormat'], $message);
            ...
            $rdNewPostID = 'http://www.reddit.com'; // prr($response);
            ...
            $r = $response['jquery'];
            if (is_array($r[$retNum][3]) && count($r[$retNum][3]) > 0 && stripos($r[$retNum][3][0], 'http://') !== false) $rdNewPostID = $r[$retNum][3][0];
