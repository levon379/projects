...
if (!class_exists("nxs_class_SNAP_BG")) {
    class nxs_class_SNAP_BG
    {
        ...
        function nsBloggerGetAuth($email, $pass)
        {
            $pass = urlencode($pass);
            ...
            $arr = explode("=", $result);
            $token = $arr[3];
            if (trim($token) == '') return false; else return $token;
        }
        ...
        function nsBloggerNewPost($auth, $blogID, $title, $text)
        {
            ...
            } else return array("code" => "ERR", "error" => $result);
        }

        function doPostToNT($options, $message)
        {
            $badOut = array('pgID' => '', 'isPosted' => 0, 'pDate' => date('Y-m-d H:i:s'), 'Error' => ''); //prr($message); prr($options);
            //## Check API Lib
            ...
            $blogID = $options['bgBlogID']; // prr($msgT); prr($msg); die();
            if (function_exists("doConnectToBlogger")) {
                $auth = doConnectToBlogger($email, $pass);
                if ($auth !== false) $ret = $auth; else $ret = doPostToBlogger($blogID, $msgT, $msg, $tags);
            } else {
                $auth = $this->nsBloggerGetAuth($email, $pass);
                if ($auth === false) $ret = 'Incorrect Username/Password'; else {
