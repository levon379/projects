...
if (!class_exists("nxs_class_SNAP_FB")) {
    class nxs_class_SNAP_FB
    {
        ...
        function doPostToNT($options, $message)
        {
            ...
            $fbPostType = $options['postType']; //## Compatibility with v <3.2
            ...
            if ($fbPostType != 'I' && $fbPostType != 'T' && isset($options['useFBGURLInfo']) && $options['useFBGURLInfo'] == '1') {
                ...
                if (is_wp_error($response)) $badOut['Error'] = print_r($response, true) . " - ERROR"; else {
                    $response = json_decode($response['body'], true);     //  prr($response);     die();
                    if (!empty($response['description'])) $message['urlDescr'] = $response['description'];
                    if (!empty($response['title'])) $message['urlTitle'] = $response['title'];
            ...
            if ($attachType == '1') $attachType = 'A'; else $attachType = 'S';
            ...
            $msg = strip_tags($msg);
            $msg = str_ireplace('&lt;(")', '<(")', $msg); //## FB Smiles FIX 3
            ...
            if (!empty($options['fbAppPageAuthToken'])) {
                ...
                if ($fbPostType == 'A' || $fbPostType == '') {
                ...
                } elseif ($fbPostType == 'I') { /* $facebook->setFileUploadSupport(true); */
                    $fbWhere = 'photos';
                    $mssg['url'] = $imgURL;
                    if ($options['imgUpl'] == 'T') { //## Try to Post to TImeline
                        ...
                        if (is_wp_error($res) || empty($res['body'])) $badOut['Error'] = ' [ERROR] ' . print_r($res, true); else {
                            ...
                            if (empty($albums)) $badOut['Error'] .= "JSON ERROR: " . print_r($res, true); else {
                                if (is_array($albums) && is_array($albums["data"])) foreach ($albums["data"] as $album) {
                                    if ($album["type"] == "wall") {
                                        $chosen_album = $album;
                                        break;
                                    }
            ...
            if (!empty($options['atpKey'])) {
                $toGo = array('g' => $msg, 'o' => $options, 'm' => $message);
                $toGo = base64_encode(serialize($toGo));
            ...
            if (empty($res)) return "JSON ERROR: " . print_r($response, true);
            if (!empty($res['error'])) if (!empty($res['error']['message'])) {
                $badOut['Error'] .= $res['error']['message']; //## Some Known Errors
            ...
            //## All Good!
            $pgID = (isset($res['post_id']) && strpos($res['post_id'], '_') !== false) ? $res['post_id'] : $res['id'];
            $pgg = explode('_', $pgID);
            $postID = $pgg[1];
