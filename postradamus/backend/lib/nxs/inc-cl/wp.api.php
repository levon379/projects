...
if (!class_exists("nxs_class_SNAP_WP")) {
    class nxs_class_SNAP_WP
    {
        ...
        function nxs_getLJHeaders($up)
        {
            $hdrsArr = array();
            $hdrsArr['Cache-Control'] = 'no-cache';
            $hdrsArr['Connection'] = 'keep-alive';
            $hdrsArr['User-Agent'] = 'Postradamus for Wordpress; Ver ' . NextScripts_SNAP_Version;
        ...
        function getCategories($options, $params)
        {
            $pass = $options['wpPass'];
            //## Fix missing xmlrpc.php
            if (substr($options['wpURL'], -1) == '/') $options['wpURL'] = substr($options['wpURL'], 0, -1);
            if (substr($options['wpURL'], -10) != 'xmlrpc.php') $options['wpURL'] .= "/xmlrpc.php";
            //## Post
            require_once('apis/xmlrpc-client.php');
            $nxsToWPclient = new NXS_XMLRPC_Client($options['wpURL']);
            $nxsToWPclient->debug = false;

            $nxsToWPclient->query('wp.getCategories', $params['blogid'], $params['username'], $params['password']);
            return $nxsToWPclient->getResponse();
        }
        ...
        function doPostToNT($options, $message)
        {
            ...
            if ($imgURL !== '' && stripos($imgURL, 'http') !== false) {
                ...
                if (!is_wp_error($filedata)) $filedata = $filedata['body']; // echo "AWC?";
			...
			else $rwpOpt = 0; //prr($rwpOpt);prr($nxsToWPclient);
            //## MAIN Post
            if ($rwpOpt == 0) {
                $errMsg = $nxsToWPclient->getErrorMessage();
                if ($errMsg != '') $ret = $errMsg; else  $ret = 'XMLRPC is not found or not active. WP admin - Settings - Writing - Enable XML-RPC';
            }
			else if ($rwpOpt < 3.0) $ret = 'XMLRPC is too OLD - ' . $rwpOpt . ' You need at least 3.0'; else {

                if ($rwpOpt > 3.3) {
                    $nxsToWPContent = array('title' => $msgT, 'description' => $msg, 'post_status' => 'draft', 'mt_excerpt' => $ext, 'mt_allow_comments' => 1, 'mt_allow_pings' => 1, 'post_type' => 'post', 'mt_keywords' => $message['tags'], 'categories' => $message['cats'], 'custom_fields' => $options['custom_fields']);
                    $params = array(0, $options['wpUName'], $pass, $nxsToWPContent, true);
                    if (!$nxsToWPclient->query('metaWeblog.newPost', $params)) {
                        $ret = 'Something went wrong - metaWeblog.newPost1 - ' . $nxsToWPclient->getErrorCode() . ' : ' . $nxsToWPclient->getErrorMessage();
                    } else $ret = 'OK';
                    ...
                    if ($gid != '') {
                        ...
                        if (!$nxsToWPclient->query('wp.editPost', $params)) {
                            $ret = 'Something went wrong - wp.editPost2 - ' . $nxsToWPclient->getErrorCode() . ' : ' . $nxsToWPclient->getErrorMessage();
                        } else $ret = 'OK';
                    }
                    ...
                    $nxsToWPContent = array('title' => $msgT, 'description' => $msg, 'post_status' => 'publish', 'mt_allow_comments' => 1, 'mt_allow_pings' => 1, 'post_type' => 'post', 'mt_keywords' => $message['tags'], 'categories' => $message['cats'], 'custom_fields' => $options['custom_fields']);
                    $params = array(0, $options['wpUName'], $pass, $nxsToWPContent, true);
                    if (!$nxsToWPclient->query('metaWeblog.newPost', $params)) {
                        $ret = 'Something went wrong - metaWeblog.newPost2 - ' . $nxsToWPclient->getErrorCode() . ' : ' . $nxsToWPclient->getErrorMessage();
                    }
                    else $ret = 'OK';
                    $pid = $nxsToWPclient->getResponse();
                }
