...
if (!class_exists("nxs_class_SNAP_IP")) {
    class nxs_class_SNAP_IP
    {
        ...
        function doPostToNT($options, $message)
        {
            ...
            if (is_wp_error($cnt)) $ret = 'Something went wrong - ' . print_r($cnt, true); else {
                if (is_array($cnt) && stripos($cnt['body'], '201') !== false) return array('postID' => 'IP', 'isPosted' => 1, 'postURL' => 'IP', 'pDate' => date('Y-m-d H:i:s'));
                    ...
                    $ret = "Error: ";
                    if (is_array($cnt) && $cnt['response']['code'] == '401') $ret .= " Incorrect Username/Password "; else $ret .= print_r($cnt, true);
