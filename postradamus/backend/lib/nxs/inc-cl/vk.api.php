...
if (!class_exists("nxs_class_SNAP_VK")) {
    class nxs_class_SNAP_VK
    {
        ...
        function doPostToNT($options, $message)
        {
            ...
            $msgOpts = array();
            $msgOpts['uid'] = $options['vkPgID']; // if ($link!='') $msgOpts['link'] = $link;
            ...
            if (function_exists('nxs_doPostToVK') && $postType == 'A' && $urlToGo != '') {
                //## Login
                if (isset($options['vkSvC'])) $nxs_vkCkArray = maybe_unserialize($options['vkSvC']);
                $loginError = true;
                if (is_array($nxs_vkCkArray)) $loginError = nxs_doCheckVK();
                if ($loginError != false) {
                    if (!empty($options['vkPh'])) {
                        $replArr = explode(' ... ', $options['vkPhReq']);
                        $ph = $options['vkPh'];
                        $ln = strlen($replArr[0]);
                        if (substr($ph, 0, $ln) == $replArr[0]) $ph = substr($ph, $ln);
                        $ln = strlen($replArr[1]);
                        $mln = -$ln;
                        if (substr($ph, $mln) == $replArr[1]) $ph = substr($ph, $ln, $mln);
                    } else $ph = '';
                    $loginError = nxs_doConnectToVK($options['uName'], $pass, $ph);
                }  //       prr($loginError);
                if ($loginError !== false) {
                    if (stripos($loginError, 'Phone verification required:') !== false) return $loginError; else return "ERROR - BAD USER/PASS - " . print_r($loginError, true);
                }
                //## Post
                $msgOpts['url'] = $urlToGo;
            ...
            if ($postType == 'I') {
                ...
                if (is_object($imgUpld)) {
                    $imgID = $imgUpld->id;
                    $atts[] = $imgID;
                } else  $badOut['Error'] .= '-=ERROR=- ' . print_r($imgUpld, true);
            }
            if ($postType != 'A') {
                if ($options['addBackLink'] == '1') $atts[] = $urlToGo;
