...
if (!class_exists("nxs_class_SNAP_LI")) {
    class nxs_class_SNAP_LI
    {
        ...
        function postToGroup($tkn, $msg, $title, $groupID, $url = '', $imgURL = '', $dsc = '')
        {
            $nURL = 'https://api.linkedin.com/v1/groups/' . $groupID . '/posts?oauth2_access_token=' . $tkn;
            $dsc = nxs_decodeEntitiesFull(strip_tags($dsc));
            $msg = strip_tags(nxs_decodeEntitiesFull($msg));
            $title = nxs_decodeEntitiesFull(strip_tags($title));
        ...
        ' . ($url != '' ? '<content><title>' . htmlspecialchars($title, ENT_NOQUOTES, "UTF-8") . '</title>' . "\n" . '<submitted-url>' . $url . '</submitted-url>' . "\n" . '<submitted-image-url>' . $imgURL . '</submitted-image-url>' . "\n" . '<description>' . htmlspecialchars($dsc, ENT_NOQUOTES, "UTF-8") . '</description></content>' : '') . '</post>';
            ...
            $wprg['sslverify'] = false;
            $response = wp_remote_post($nURL, $wprg);
            if (is_wp_error($response) || $response['response']['code'] != '201') return "ERROR: " . print_r($response, true);
        ...
        function doPostToNT($options, $message)
        {
            ...
            if (empty($options['liMsgFormatT'])) $options['liMsgFormatT'] = '%TITLE%';
            //## Format
            ...
            if (function_exists("doConnectToLinkedIn") && $options['ulName'] != '' && $options['uPass'] != '') {
                ...
                $pass = (substr($options['uPass'], 0, 5) == 'n5g9a' ? nsx_doDecode(substr($options['uPass'], 5)) : $options['uPass']); // ??? Do we need that??????
                ...
                if ($options['isV2']) {  //## V2
                    ...
                    if ($options['grpID'] != '') {
                        try {
                            ...
                            $liPostID = 'http://www.linkedin.com/groups?gid=' . $options['grpID'];
                            if ($ret == '201') $ret = array('updateUrl' => $liPostID);
                        } catch (Exception $o) {
                            $ret = "ERROR: " . print_r($o, true);
                        }
                    } else { //echo $msg ."|". nsTrnc($msgT, 200) ."|". $urlToGo ."|". $imgURL ."|". $dsc;
                        try {
                            if ($options['liAttch'] == '1') $ret = $linkedin->postShare($msg, nsTrnc($msgT, 200), str_replace('&', '&amp;', $urlToGo), $imgURL, $dsc); else $ret = $linkedin->postShare($msg);
                        } catch (Exception $o) {
                            $ret = "ERROR:" . print_r($o, true);
                        }
                    }
            ...
            if (is_array($ret) && !empty($ret['updateUrl'])) {
                if (stripos($ret['updateUrl'], 'topic=') !== false) $liPostID = CutFromTo($ret['updateUrl'], 'topic=', '&'); else $liPostID = '';
