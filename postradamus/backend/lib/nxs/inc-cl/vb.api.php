...
if (!class_exists("nxs_class_SNAP_VB")) {
    class nxs_class_SNAP_VB
    {
        ...
        function nxs_getVBHeaders($ref, $post = false)
        {
            $hdrsArr = array();
        ...
        function nxs_doCheckVB($url)
        {
            ...
            if (stripos($response['body'], 'usercp.php') !== false && stripos($response['body'], 'logouthash') !== false) { /*echo "You are IN"; */
                return false;
        ...
        function nxs_doConnectToVB($u, $p, $url)
        {
            ...
            if (stripos($contents, 'base href="') !== false) $baseURL = trim(CutFromTo($contents, 'base href="', '"')); else {
                $uarr = explode('/', $url);
            ...
            if (stripos($r2['body'], 'exec_refresh()') !== false) {
                ...
                $nxs_vbCkArray = $ckArr;
                return false;
            } else return "Bad Username/Password";
        }
        ...
        function nxs_doPostToVB($url, $subj, $msg, $lnk, $tags)
        {
            ...
            if (stripos($contents, 'newthread.php?do=newthread') !== false) $mdd = 't'; elseif (stripos($contents, 'newreply.php?') !== false) $mdd = 'p';
            else return "No Thread/Post Controls found";

            if ($mdd == 't') {
                $fid = CutFromTo($contents, 'newthread.php?do=newthread', '"'); // echo  $baseURL.'newthread.php?do=newthread'.str_replace('&amp;','&',$fid);
        ...
        function doPostToNT($options, $message)
        {
            ...
            //## Post
            if (isset($options['vbSvC'])) $nxs_vbCkArray = maybe_unserialize($options['vbSvC']);
            $loginError = true;
            if (is_array($nxs_vbCkArray)) $loginError = $this->nxs_doCheckVB($options['vbURL']);
            if ($loginError !== false) $loginError = $this->nxs_doConnectToVB($options['vbUName'], $pass, $options['vbURL']);
