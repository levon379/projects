...
if (!function_exists("doConnectToDeviantART")) {
    function doConnectToDeviantART($unm, $pass)
    {
    }
}

if (!class_exists('nxsAPI_DA')) {
    class nxsAPI_DA
    {
        var $ck = array();
        var $mh = '';
        ...
        function check()
        {
            ...
            if (!empty($ck) && is_array($ck)) {
                ...
                if ($this->debug) echo "[DA] Checking....;<br/>\r\n";
                ...
                $ck2 = $rep['cookies'];
                for ($i = 0; $i < count($ck); $i++) if ($ck[$i]->name == 'userinfo') $ck[$i]->value = urlencode($ck2[0]->value);
                $this->ck = $ck;
        ...
        function connect($u, $p)
        {
            ...
            //## Check if alrady IN
            if (!$this->check()) {
                if ($this->debug) echo "[DA] NO Saved Data;<br/>\r\n";
                $url = "https://www.deviantart.com/users/login";
                $hdrsArr = $this->headers('http://www.deviantart.com/');
                $rep = wp_remote_get($url, array('headers' => $hdrsArr, 'httpversion' => '1.1'));
                if (is_wp_error($rep)) {
                    $badOut = print_r($rep, true) . " - ERROR Login 1";
                    return $badOut;
                }
                $ck = $rep['cookies'];
                $rTok = CutFromTo($rep['body'], 'name="validate_token" value="', '"');
                $rKey = CutFromTo($rep['body'], 'name="validate_key" value="', '"');
                $ck[0]->value = urlencode($ck[0]->value);
                ...
                if (is_wp_error($response)) {
                    $badOut = print_r($response, true) . " - ERROR Login 2";
                    return $badOut;
                }
                $ck = $response['cookies'];
                for ($i = 0; $i < 4; $i++) $ck[$i]->value = urlencode($ck[$i]->value);
                if (isset($response['headers']['location']) && stripos($response['headers']['location'], 'wrong-password') !== false) {
                    $badOut = "Wrong Password - ERROR";
                    return $badOut;
                }
                if (isset($response['headers']['location']) && ($response['headers']['location'] == 'http://www.deviantart.com' || $response['headers']['location'] == 'https://www.deviantart.com/users/loggedin')) {
                    $hdrsArr = $this->headers('http://www.deviantart.com');
                    $rep = wp_remote_get('http://www.deviantart.com', array('headers' => $hdrsArr, 'httpversion' => '1.1', 'cookies' => $ck));// die();  prr($rep);
                    if (is_wp_error($rep)) {
                        $badOut = print_r($rep, true) . " - ERROR  Login 3";
                        return $badOut;
                    }
                    $mh = CutFromTo($rep['body'], 'Your Account</b><br><a href="', '"');
                    $mh = str_ireplace('/journal/', '', $mh);
                    $ck2 = $rep['cookies'];
                    for ($i = 0; $i < count($ck); $i++) if ($ck[$i]->name == 'userinfo') $ck[$i]->value = urlencode($ck2[0]->value);
                    $this->ck = $ck;
                    $this->mh = $mh;
                    return false;
                } else  $badOut = print_r($response, true) . " - ERROR  Login 4";
                return $badOut;
            } else {
                if ($this->debug) echo "[DA] Saved Data is OK;<br/>\r\n";
                return false;
            }
        }
        ...
        function post($post)
        {
            ...
            $fldsOut = $flds;
            $ckk = array();
            for ($i = 0; $i < count($ck); $i++) if ($ck[$i]->name == 'userinfo' || $ck[$i]->name == 'auth') $ckk[] = $ck[$i];
            $ck = $ckk;
            sleep(16); //## Important.
...
if (!class_exists("nxs_class_SNAP_DA")) {
    class nxs_class_SNAP_DA
    {
        ...
        function doPostToNT($options, $message)
        {
            ...
            //## Format Post
            if (!empty($message['pTitle'])) $title = $message['pTitle']; else $title = nxs_doFormatMsg($options['daTitleFormat'], $message);
