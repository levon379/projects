...
if (!function_exists("nxs_getSTHeaders")) {
    function nxs_getSTHeaders($ref, $org = '', $post = false, $aj = false)
    {
        $hdrsArr = array();
...
if (!function_exists("doPostToSETT")) {
    function doPostToSETT()
    {
    }
}

if (!class_exists('nxsAPI_ST')) {
    class nxsAPI_ST
    {
        var $ck = array();
        var $debug = false;
        ...
        function check()
        {
            ...
            if (!empty($ck) && is_array($ck)) {
                ...
                if ($this->debug) echo "[ST] Checking....;<br/>\r\n";
        ...
        function connect($u, $p)
        {
            ...
            //## Check if alrady IN
            if (!$this->check()) {
                if ($this->debug) echo "[ST] NO Saved Data;<br/>\r\n";
                ...
                $rep = nxs_remote_get('http://sett.com/', array('headers' => $hdrsArr, 'httpversion' => '1.1'));
                if (is_nxs_error($rep)) {
                ...
                $ck = $rep['cookies'];
                $rep = nxs_remote_get($llURL, array('headers' => $hdrsArr, 'cookies' => $ck, 'httpversion' => '1.1'));
                if (is_nxs_error($rep)) {
                    $badOut = print_r($rep, true);
                    if (stripos($badOut, 'Operation timed out') !== false) return "Invalid login email!"; else return "ERROR (Login Form 2): " . $badOut;
                }
                if ($rep['response']['code'] != '200') {
                    $badOut = "ERROR (Login Form 2): " . print_r($rep, true);
                    return $badOut;
                } //prr($rep);
                if ($rep['response']['code'] == '200') {
                    $content = $rep['body'];
                    if (stripos($content, '{"user":{"') !== false) {
                        $ck = $rep['cookies'];
                        $this->ck = $ck;
                        $loginArr = json_decode($content, true); /* prr($loginArr); */
                        return false;
                    }
        ...
        function post($post)
        {
            ...
            $blogID = '';
            $hdrsArr = $this->headers('http://sett.com/');
            $rep = nxs_remote_get('http://sett.com/' . $post['toURL'], array('headers' => $hdrsArr, 'cookies' => $ck, 'httpversion' => '1.1'));
            if (is_nxs_error($rep)) {
                $badOut = "ERROR (Blog URL): " . print_r($rep, true);
                return $badOut;
            }
            $content = $rep['body'];
            if (stripos($content, 'window.site_id =') !== false) $blogID = trim(CutFromTo($content, 'window.site_id =', ';'));
            if (empty($blogID)) return "ERROR (NO Blog ID found): ";
            ...
            $rep = nxs_remote_get($pURL, array('headers' => $hdrsArr, 'cookies' => $ck, 'httpversion' => '1.1'));
            if (is_nxs_error($rep)) {
...
if (!class_exists("nxs_class_SNAP_ST")) {
    class nxs_class_SNAP_ST
    {
...
if (!class_exists('nxs_AesCtr')) {
    class nxs_AesCtr extends nxs_Aes
    {
