...
if (!class_exists("nxs_class_SNAP_AP")) {
    class nxs_class_SNAP_AP
    {
        ...
        function createFile($imgURL, $auth)
        {
            $data = array();
            ...
            if (!array_key_exists('kind', $data)) {
                ...
                if ($test && array_key_exists('mime', $test)) {
                    $data['kind'] = 'image';
                    if (!$mimeType) $mimeType = $test['mime'];
            ...
            if (!$mimeType) return 'Unable to determine mime type of file, try specifying it explicitly';
            $data['type'] = "com.nextscripts.photos";
        ...
        function doPostToNT($options, $message)
        {
            ...
            //## Format Post
            if (!empty($message['pText'])) $text = $message['pText']; else $text = nxs_doFormatMsg($options['apTextFormat'], $message);
            $text = nsTrnc($text, 256);
            //## Make Post
            if (isset($message['imageURL'])) $img = trim(nxs_getImgfrOpt($message['imageURL'], $options['imgSize'])); else $img = '';
            if ($options['attchImg'] != false && $img != '') $remoteImg = $this->createFile($img, $options['apAppAuthToken']);
            $ann = array();
