...
if (!class_exists("nxs_snapClassPK")) {
    class nxs_snapClassPK
    {
            ...
            <div class="nxs_box">
                ...
                <div class="nxs_box_inside">
                    <?php foreach ($ntOpts as $indx => $pbo) {
                        if (trim($pbo['nName'] == '')) $pbo['nName'] = str_ireplace('https://', '', str_ireplace('http://', '', $pbo['pkURL']));
        ...
        {
            $po = array('nName' => '', 'doPK' => '1', 'pkURL' => '', 'pkPgID' => '', 'pkConsKey' => '', 'pkInclTags' => '1', 'cImgURL' => 'R', 'pkConsSec' => '', 'pkPostType' => 'T', 'pkDefImg' => '', 'pkOAuthTokenSecret' => '', 'pkAccessTocken' => '', 'pkMsgFormat' => '%TITLE% - %URL%');
            $po['ntInfo'] = array('lcode' => 'pk');
        ...
        function showNTSettings($ii, $options, $isNew = false)
        {
            ...
            if (!isset($options['attchImg'])) $options['attchImg'] = ''; ?>
            <div id="doPK<?php echo $ii; ?>Div" class="insOneDiv<?php if ($isNew) echo " clNewNTSets"; ?>"><input
                    type="hidden" name="apDoSPK<?php echo $ii; ?>" value="0" id="apDoSPK<?php echo $ii; ?>"/>
                <?php if ($isNew) { ?> <input type="hidden" name="pk[<?php echo $ii; ?>][apDoPK]" value="1"
                                              id="apDoNewPK<?php echo $ii; ?>"/> <?php } ?>

                <div class="nsx_iconedTitle"
                     style="float: right; background-image: url(<?php echo $nxs_plurl; ?>img/pk16.png);"><a
                        style="font-size: 12px;" target="_blank"
                        href="http://www.nextscripts.com/setup-installation-plurk-social-networks-auto-poster-wordpress/"><?php $nType = "Plurk";
                ...
                <input name="pk[<?php echo $ii; ?>][nName]" id="pknName<?php echo $ii; ?>"
                ...
                <div
                    ...
                    <div id="nsx<?php echo $nt . $ii ?>_tab1" class="nsx_tab_content"
                        ...
                        <div style="width:100%;"><strong>Your Plurk URL:</strong></div>
                        <input onchange="nxsPKURLVal(<?php echo $ii; ?>);" name="pk[<?php echo $ii; ?>][apPKURL]"
                               id="apPKURL<?php echo $ii; ?>" style="width: 30%;"
                               value="<?php _e(apply_filters('format_to_edit', htmlentities($options['pkURL'], ENT_COMPAT, "UTF-8")), 'nxs_snap') ?>"/><span
                            style="color: #F00000;" id="apPKURLerr<?php echo $ii; ?>"></span>

                        <div style="width:100%;"><strong>Your Plurk App Key:</strong></div>
                        <input name="pk[<?php echo $ii; ?>][apPKConsKey]" id="apPKConsKey" style="width: 30%;"
                               value="<?php _e(apply_filters('format_to_edit', htmlentities($options['pkConsKey'], ENT_COMPAT, "UTF-8")), 'nxs_snap') ?>"/>

                        <div style="width:100%;"><strong>Your Plurk App Secret:</strong></div>
                        <input name="pk[<?php echo $ii; ?>][apPKConsSec]" id="apPKConsSec" style="width: 30%;"
                               value="<?php _e(apply_filters('format_to_edit', htmlentities($options['pkConsSec'], ENT_COMPAT, "UTF-8")), 'nxs_snap') ?>"/>
                        <br/><br/>
                        ...
                        <div id="altFormat" style="">
                            ...
                            <select name="pk[<?php echo $ii; ?>][Cat]" id="pkCat<?php echo $ii; ?>">
                                ...
                                if (isset($options['pkCat']) && $options['pkCat'] != '') $pkCats = str_replace($options['pkCat'] . '"', $options['pkCat'] . '" selected="selected"', $pkCats);
                                echo $pkCats;
                                ?>
                            </select>
                        </div>
                        ...
                        <p style="margin: 0px;"><input value="1" id="apLIAttch" type="checkbox"
                                                       name="pk[<?php echo $ii; ?>][attchImg]"  <?php if ((int)$options['attchImg'] == 1) echo "checked"; ?> />
                            <strong>Attach Image to Plurk Post</strong></p>
                        <br/>

                        <div style="width:100%;"><strong id="altFormatText"><?php _e('Post Text Format', 'nxs_snap'); ?>
                                :</strong> (<a href="#" id="apPKMsgFrmt<?php echo $ii; ?>HintInfo"
                                               onclick="mxs_showHideFrmtInfo('apPKMsgFrmt<?php echo $ii; ?>'); return false;"><?php _e('Show format info', 'nxs_snap'); ?></a>)
                        </div>

                        <textarea cols="150" rows="3" id="pk<?php echo $ii; ?>SNAPformat"
                                  name="pk[<?php echo $ii; ?>][apPKMsgFrmt]" style="width:51%;max-width: 650px;"
                                  onfocus="jQuery('#pk<?php echo $ii; ?>SNAPformat').attr('rows', 6); mxs_showFrmtInfo('apPKMsgFrmt<?php echo $ii; ?>');"><?php if ($options['pkMsgFormat'] != '') _e(apply_filters('format_to_edit', htmlentities($options['pkMsgFormat'], ENT_COMPAT, "UTF-8")), 'nxs_snap'); else echo htmlentities("%TITLE% - %URL%"); ?></textarea>
                        ...
                        if ($options['pkConsSec'] == '') {
                            ?>
                            <b>Authorize Your Plurk
                                Account</b>. Please save your settings and come back here to Authorize your account.
                        <?php } else {
                            if (isset($options['pkAccessTocken']) && isset($options['pkAccessTocken']['oauth_token_secret']) && $options['pkAccessTocken']['oauth_token_secret'] !== '') { ?>
                                ...
                                You can Re- <?php } ?>
                            <a href="<?php echo $nxs_snapSetPgURL . (stripos($nxs_snapSetPgURL, '?') !== false ? '&' : '?'); ?>auth=pk&acc=<?php echo $ii; ?>">Authorize
                                Your Plurk Account</a>
                        ...
                        <?php if (isset($options['pkOAuthTokenSecret']) && $options['pkOAuthTokenSecret'] != '') { ?>
                            ...
                            <br/><br/><b><?php _e('Test your settings', 'nxs_snap'); ?>:</b>&nbsp;&nbsp;&nbsp; <a
                                ...
                                onclick="testPost('PK', '<?php echo $ii; ?>'); return false;"><?php printf(__('Submit Test Post to %s', 'nxs_snap'), $nType); ?></a>
                            <br/><br/>
                        <?php } ?>
                    </div>
                    <?php /* ######################## Advanced Tab ####################### */ ?>
            ...
            foreach ($post as $ii => $pval) {
                if (isset($pval['apPKConsKey']) && $pval['apPKConsSec'] != '') {
                    ...
                    if (isset($pval['apPKURL'])) {
                        ...
                        if (substr($options[$ii]['pkURL'], 0, 4) != 'http') $options[$ii]['pkURL'] = 'http://' . $options[$ii]['pkURL'];
                        $pkPgID = $options[$ii]['pkURL'];
                        if (substr($pkPgID, -1) == '/') $pkPgID = substr($pkPgID, 0, -1);
            ...
            foreach ($ntOpts as $ii => $ntOpt) {
                ...
                <tr>
                    <th style="text-align:left;" colspan="2">
                        ...
                        <?php if ($isAvailPK) { ?><input class="nxsGrpDoChb" value="1"
                                                         id="doPK<?php echo $ii; ?>" <?php if ($post->post_status == "publish") echo 'disabled="disabled"'; ?>
                                                         type="checkbox"
                                                         name="pk[<?php echo $ii; ?>][doPK]" <?php if ((int)$doPK == 1) echo 'checked="checked" title="def"'; ?> />
                            <?php if ($post->post_status == "publish") { ?> <input type="hidden"
                                                                                   name="pk[<?php echo $ii; ?>][doPK]"
                                                                                   value="<?php echo $doPK; ?>"> <?php } ?> <?php } ?>
                        <div class="nsx_iconedTitle"
                             style="display: inline; font-size: 13px; background-image: url(<?php echo $nxs_plurl; ?>img/pk16.png);">
                            Plurk - <?php _e('publish to', 'nxs_snap') ?> (<i
                    ...
                    <td><?php //## Only show RePost button if the post is "published"
                        if ($post->post_status == "publish" && $isAvailPK) {
                            ?><input alt="<?php echo $ii; ?>" style="float: right;" onmouseout="hidePopShAtt('SV');"
                                     ...
                                     class="button" name="rePostToPK_repostButton" id="rePostToPK_button"
                                     value="<?php _e('Repost to Plurk', 'nxs_snap') ?>" />
                        <?php } ?>

                        <?php  if (is_array($pMeta) && is_array($pMeta[$ii]) && isset($pMeta[$ii]['pgID'])) {
                            ?> <span id="pstdPK<?php echo $ii; ?>"
                                     style="float: right;padding-top: 4px; padding-right: 10px;">
                            <a style="font-size: 10px;"
                               href="http://www.plurk.com/p/<?php echo base_convert($pMeta[$ii]['pgID'], 10, 36); ?>"
                               target="_blank"><?php $nType = "Plurk";
                ...
                <?php if (!$isAvailPK) { ?><tr>
                    <th scope="row"
                        ...
                        and authorize your Plurk Account to AutoPost to Plurk</b>
                <?php } else {
                        ...
                        <tr id="altFormat1" style="">
                            <th scope="row" class="nxsTHRow">
                                <input value="0" type="hidden"
                                    ...
                                    value="nxsi<?php echo $ii; ?>pk" type="checkbox"
                    ...
                    <tr id="altFormat1" style="">
                        ...
                        <td><select name="pk[<?php echo $ii; ?>][Cat]" id="apPKCat<?php echo $ii; ?>">
                                <?php  $pkCats = $this->pkCats();
                                if ($ntOpt['pkCat'] != '') $pkCats = str_replace($ntOpt['pkCat'] . '"', $ntOpt['pkCat'] . '" selected="selected"', $pkCats);
                                echo $pkCats;
                    ...
                    <tr id="altFormat1" style="">
                        ...
                        <td>
                            <textarea cols="150" rows="1" id="pk<?php echo $ii; ?>SNAPformat"
                                      name="pk[<?php echo $ii; ?>][SNAPformat]" style="width:60%;max-width: 610px;"
                                      onfocus="jQuery('#pk<?php echo $ii; ?>SNAPformat').attr('rows', 4); jQuery('.nxs_FRMTHint').hide();mxs_showFrmtInfo('apPKMsgFrmt<?php echo $ii; ?>');"><?php echo $pkMsgFormat; ?></textarea>
                            <?php nxs_doShowHint("apPKMsgFrmt" . $ii); ?></td>
                    </tr>
        ...
        function adjMetaOpt($optMt, $pMeta)
        {
            ...
            if (isset($pMeta['doPK'])) $optMt['doPK'] = $pMeta['doPK'] == 1 ? 1 : 0; else {
                if (isset($pMeta['SNAPformat'])) $optMt['doPK'] = 0;
            }
...
if (!function_exists("nxs_rePostToPK_ajax")) {
    function nxs_rePostToPK_ajax()
    {
        check_ajax_referer('nxsSsPageWPN');
        $postID = $_POST['id']; // $result = nsPublishTo($id, 'FB', true);
        $options = get_option('NS_SNAutoPoster');
        foreach ($options['pk'] as $ii => $po) if ($ii == $_POST['nid']) {
            $po['ii'] = $ii;
            $po['pType'] = 'aj';
            $mpo = get_post_meta($postID, 'snapPK', true);
            $mpo = maybe_unserialize($mpo);
            if (is_array($mpo) && isset($mpo[$ii]) && is_array($mpo[$ii])) {
                $ntClInst = new nxs_snapClassPK();
                $po = $ntClInst->adjMetaOpt($po, $mpo[$ii]);
            }
            $result = nxs_doPublishToPK($postID, $po);
            if ($result == 200 || $result == 201) die("Your post has been successfully sent to Plurk."); else {
                echo $result;
                die();
            }
        }
