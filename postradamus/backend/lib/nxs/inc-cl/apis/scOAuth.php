...
class wpScoopITOAuth
{
    ...
    function get_normalized_parameters($params)
    {
        $items = array();
    ...
    function genRndString($length = 8)
    {
        $chars = '0123456789abcdefghijklmnopqrstuvwxyzz';
        $string = '';
        for ($p = 0; $p < $length; $p++) $string .= $chars[mt_rand(0, strlen($chars) - 1)];
        return $string;
    }

    function makeHTTPHeaders($ref, $post = false)
    {
        $hdrsArr = array();
    ...
    function getAccToken($verifier)
    {
        ...
        $req = array();
        $req['method'] = 'GET';
        $req['normalized_url'] = $this->baseURL . $this->access_token_path; // echo "ARGS:"; prr($args);
        $req['normalized_parameters'] = $this->get_normalized_parameters($args);
    ...
    function makeReq($url, $params, $type = 'GET')
    {
        ...
        if (is_array($params)) {
            $argsTS = array_merge($args, $params);
        } else $argsTS = $args;
        $req = array();
        $req['method'] = $type;
        ...
        if (is_array($params)) {
            if (isset($params['tag']) && is_array($params['tag'])) {
                ...
                unset($params['tag']);
                $argsAddStr = '';
                foreach ($tags as $tg) $argsAddStr .= '&tag=' . urlencode($tg);
            }
            $params = nxssc_SigMethod_HMAC_SHA1::urlencode_rfc3986($params);
            $args = array_merge($args, $params);
        }
        $argsStr = '';
        $argsT = array();
        foreach ($args as $arN => $arV) {
            $argsT[] = $arN . '=' . $arV;
        }
        $argsStr = implode('&', $argsT);
        ...
        if ($type == 'GET') {
            ...
            $hdrsArr = $this->makeHTTPHeaders($url, true);
            if (!empty($argsAddStr)) $argsStr .= $argsAddStr;
        ...
        if (is_array($body)) return $body; else  return json_decode($body, true);
    }
    ...
    private function joinParameters($parameters)
    {
        ...
        $keyValuePairs = array();
        foreach ($keys as $k) array_push($keyValuePairs, rawurlencode($k) . "=" . rawurlencode($parameters[$k]));
    ...
    public function flUploadPhoto($imgFile, $parameters)
    {
        ...
        $requestParams['photo'] = $imgFile; // prr($requestParams);
