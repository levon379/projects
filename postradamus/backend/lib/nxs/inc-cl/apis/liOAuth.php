...
class nsx_LinkedIn
{
    ...
    function getRequestToken()
    {
        ...
        $this->request_token = new nsx_trOAuthConsumer($response_params['oauth_token'], $response_params['oauth_token_secret'], 1);
    ...
    {
        $status_url = $this->base_url . "/v1/groups/" . $groupID . "/posts";  //$debug = true;
        $dsc = nxs_decodeEntitiesFull(strip_tags($dsc));
        $msg = strip_tags(nxs_decodeEntitiesFull($msg));
        $title = nxs_decodeEntitiesFull(strip_tags($title));
    ...
    { // $this->debug = true; //if (!is_array($auth_header)) $auth_header = array($auth_header);
        ...
        if ($nxs_skipSSLCheck === true) curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
        $data = curl_exec($curl);
        $errmsg = curl_error($curl); //prr($data);// die();
