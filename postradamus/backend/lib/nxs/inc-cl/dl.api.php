...
if (!class_exists("nxs_class_SNAP_DL")) {
    class nxs_class_SNAP_DL
    {
        ...
        function doPostToNT($options, $message)
        {
            ...
            if (!empty($message['pTitle'])) $msgT = $message['pTitle']; else $msgT = nxs_doFormatMsg($options['dlMsgTFormat'], $message);
            ...
            $hdrsArr = $this->nxs_getDLHeaders($email . ':' . $pass);
            $cnt = wp_remote_get($apicall, array('method' => 'GET', 'timeout' => 45, 'redirection' => 0, 'headers' => $hdrsArr)); // prr($cnt);
            ...
            if (is_wp_error($cnt)) {
                ...
                elseif (is_array($cnt) && stripos($cnt['body'], 'item already exists') !== false) $ret = '..All good, but this link has already been bookmarked..';
                    ...
                    if ($cnt['response']['code'] == '401') $ret = " Incorrect Username/Password "; else  $ret = 'Something went wrong - ' . "https://$email:*********@$api/posts/add?&url=$link&description=$desc&extended=$ext&tags=$tags | ERR: " . print_r($cnt, true);
                }
            }
            $badOut['Error'] .= $ret;
            return $badOut;
