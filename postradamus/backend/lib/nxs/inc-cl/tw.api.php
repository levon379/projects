...
if (!class_exists("nxs_class_SNAP_TW")) {
    class nxs_class_SNAP_TW
    {
        ...
        function doPostToNT($options, $message)
        {
            ...
            //## Old Settings Fix
            if ($options['attchImg'] == '1') $options['attchImg'] = 'large';
            if ($options['attchImg'] == '0') $options['attchImg'] = false;
            ...
            if ($options['attchImg'] != false && $img == '' && $imgURL != '') {
                ...
                if (ini_get('allow_url_fopen')) {
                    if (getimagesize($imgURL) !== false) {
                        ...
                        if (is_nxs_error($img)) $options['attchImg'] = false; else $img = $img['body'];
                    } else $options['attchImg'] = false;
                } else {
                    $img = nxs_remote_get($imgURL);
                    if (is_nxs_error($img)) $options['attchImg'] = false; elseif (isset($img['body']) && trim($img['body']) != '') $img = $img['body'];
                    else $options['attchImg'] = false;
                }
            }
            ...
            if ($code == 200) {
                ...
                if (is_array($twResp) && isset($twResp['id_str'])) $twNewPostID = $twResp['id_str'];
