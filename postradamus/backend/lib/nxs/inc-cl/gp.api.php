...
if (!class_exists("nxs_class_SNAP_GP")) {
    class nxs_class_SNAP_GP
    {
        ...
        function doPostToNT($options, $message)
        {
            ...
            if ($gpPostType == 'I') $lnk = array();
            if ($gpPostType == 'A') $lnk = doGetGoogleUrlInfo2($message['url']);
            if (is_array($lnk) && $imgURL != '') $lnk['img'] = $imgURL;
            ...
            } elseif (!empty($options['gpCommID'])) $ret = doPostToGooglePlus2($msg, $lnk, $options['gpPageID'], $options['gpCommID'], $options['gpCCat']);
            else $ret = doPostToGooglePlus2($msg, $lnk);
