...
if (!class_exists("nxs_class_SNAP_SU")) {
    class nxs_class_SNAP_SU
    {
        ...
        function nxs_getSUHeaders($ref, $post = false, $xhr = true)
        {
            $hdrsArr = array();
        ...
        function nxs_doCheckSU()
        {
            global $nxs_suCkArray;
            $hdrsArr = $this->nxs_getSUHeaders('https://www.stumbleupon.com/submit');
            $ckArr = $nxs_suCkArray;
        ...
        {
            global $nxs_suCkArray;
            $hdrsArr = $this->nxs_getSUHeaders('https://www.stumbleupon.com/', false, false); //   echo "LOGGIN";
            ...
            $ckArr = $response['cookies']; //$response['body'] = htmlentities($response['body']);  prr($response);    die();
        ...
        function nxs_doPostToSU($msg, $lnk, $cat, $tags, $nsfw = false)
        {
            ...
            if ($resp['_success'] == '1') {
                ...
                return $resp['_reason'];
            } else return "ERROR" . print_r($resp, true);
        }
        ...
        function doPostToNT($options, $message)
        {
            ...
            //## Format
            if (!empty($message['pText'])) $msg = $message['pText']; else $msg = nxs_doFormatMsg($options['suMsgFormat'], $message);
            $urlToGo = (!empty($message['url'])) ? $message['url'] : '';
            $tags = $message['tags'];

            if (isset($options['suSvC'])) $nxs_suCkArray = maybe_unserialize($options['suSvC']);
            $loginError = true;
            if (is_array($nxs_suCkArray)) $loginError = $this->nxs_doCheckSU();
            if ($loginError != false) $loginError = $this->nxs_doConnectToSU($options['suUName'], $pass);
            if ($loginError !== false) {
            ...
            } elseif (isset($ret['code']) && $ret['code'] == 'OK') return array('isPosted' => '1', 'postID' => $ret['post_id'], 'postURL' => 'http://www.stumbleupon.com/content/' . $ret['post_id'] . '/comments', 'pDate' => date('Y-m-d H:i:s'));
            else $badOut['Error'] .= 'Error - ' . print_r($ret, true);
            return $badOut;
        }
    }
}
