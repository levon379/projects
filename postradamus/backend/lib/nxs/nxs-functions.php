...
if (!function_exists('nsTrnc')) {
    function nsTrnc($string, $limit, $break = " ", $pad = " ...")
    {
        if (strlen($string) <= $limit) return $string;
        $string = substr($string, 0, $limit - strlen($pad));
        $brLoc = strripos($string, $break);
        if ($brLoc === false) return $string . $pad; else return substr($string, 0, $brLoc) . $pad;
    }
}
...
if (!function_exists('nsx_doEncode')) {
    function nsx_doEncode($string, $key = 'NSX')
    {
        $key = sha1($key);
...
if (!function_exists('nsx_doDecode')) {
    function nsx_doDecode($string, $key = 'NSX')
    {
        $key = sha1($key);
        $strLen = strlen($string);
        $keyLen = strlen($key);
        $j = 0;
        $hash = '';
        $strA = str_split($string, 2);
        foreach ($strA as $ordStr) {
            $ordStr = hexdec(base_convert(strrev($ordStr), 36, 16));
            if ($j == $keyLen) $j = 0;
            $ordKey = ord(substr($key, $j, 1));
            $j++;
...
if (!function_exists('nsx_stripSlashes')) {
    function nsx_stripSlashes(&$value)
    {
        $value = stripslashes($value);
    }
}
if (!function_exists('nsx_fixSlashes')) {
    function nsx_fixSlashes(&$value)
    {
...
if (!function_exists('nxs_strLen')) {
    ...
    {
        return count(str_split(utf8_decode($str)));
    }
}
if (!function_exists('nxs_currPageURL')) {
    function nxs_currPageURL()
    {
        if (empty($_SERVER['REQUEST_URI'])) $_SERVER['REQUEST_URI'] = $_SERVER['PHP_SELF'] . '?' . $_SERVER['argv'][0];
...
if (!function_exists('nxs_settings_open')) {
    ...
    {
        $fileData = trim(file_get_contents(dirname(__FILE__) . '/nx-snap-settings.txt'));
        $options = nxs_maybe_unserialize($fileData);
        return $options;
    }
}
...
if (!function_exists("nxs_mkShortURL")) {
    function nxs_mkShortURL($url, $postID = '')
    {
        return $url;
    }
}
if (!function_exists("nxs_spinRecursion")) {
    function nxs_spinRecursion(&$txt, $startCh)
    {
        global $nxs_spin_lCh, $nxs_spin_rCh, $nxs_spin_splCh;
        $startPos = $startCh;
        while ($startCh++ < strlen($txt)) {
            ...
            elseif (substr($txt, $startCh, strlen($nxs_spin_rCh)) == $nxs_spin_rCh) {
                ...
                $toRepl = nxs_spinReplace($tmpTxt);
                $txt = str_replace($nxs_spin_lCh . $tmpTxt . $nxs_spin_rCh, $toRepl, $txt);
            }
        }
        return $txt;
    }
}
if (!function_exists("nxs_spinReplace")) {
    function nxs_spinReplace($txt)
    {
        global $nxs_spin_splCh;
        $txt = explode($nxs_spin_splCh, $txt);
        $out = $txt[mt_rand(0, count($txt) - 1)];
        return $out;
    }
}
if (!function_exists("nxs_doSpin")) {
    function nxs_doSpin($msg)
    {
        global $nxs_spin_lCh, $nxs_spin_rCh, $nxs_spin_splCh;
        $nxs_spin_lCh = '{';
        $nxs_spin_rCh = '}';
        $nxs_spin_splCh = '|';
        $msg = nxs_spinRecursion($msg, -1);
        return $msg;
    }
}
if (!function_exists("nxs_getImgfrOpt")) {
    ...
    {
        if (!is_array($imgOpts)) return $imgOpts;// prr($imgOpts);
...
if (!function_exists('nxs_doFormatMsg')) {
    function nxs_doFormatMsg($format, $message, $addURLParams = '')
    {
        global $nxs_urlLen;
        $msg = nxs_doSpin($format);// prr($msg); prr($message);// Make "message default"
        $mdef = array('title' => '', 'announce' => '', 'text' => '', 'url' => '', 'surl' => '', 'urlDescr' => '', 'urlTitle' => '', 'imageURL' => '', 'videoCode' => '', 'videoURL' => '', 'siteName' => '', 'tags' => '',
            'cats' => '', 'authorName' => '', 'orID' => '');
        $message = array_merge($mdef, $message);
        if (preg_match('%URL%', $msg)) {
            $url = $message['url'];
            if ($addURLParams != '') $url .= (strpos($url, '?') !== false ? '&' : '?') . $addURLParams;
            $nxs_urlLen = nxs_strLen($url);
            $msg = str_ireplace("%URL%", $url, $msg);
        }
        if (preg_match('%SURL%', $msg)) {
            if (isset($message['surl']) && $message['surl'] != '') $url = $message['surl']; else {
                $url = $message['url'];
                if ($addURLParams != '') $url .= (strpos($url, '?') !== false ? '&' : '?') . $addURLParams;
        ...
        if (preg_match('%IMG%', $msg)) {
            if (isset($message['imageURL']) && is_array($message['imageURL'])) {
                $imgURL = trim($message['imageURL']['large']);
                if ($imgURL == '') $imgURL = trim($message['imageURL']['medium']);
                if ($imgURL == '') $imgURL = trim($message['imageURL']['original']);
                if ($imgURL == '') $imgURL = trim($message['imageURL']['thumb']);
            } elseif (!empty($message['imageURL'])) $imgURL = $message['imageURL'];
            else $imgURL = '';
            $msg = str_ireplace("%IMG%", $imgURL, $msg);
        }
        if (preg_match('%IMGLARGE%', $msg)) $msg = str_ireplace("%IMG%", trim($message['imageURL']['large'], $msg));
        if (preg_match('%IMGMEDIUM%', $msg)) $msg = str_ireplace("%IMGMEDIUM%", trim($message['imageURL']['medium'], $msg));
        if (preg_match('%IMGTHUMB%', $msg)) $msg = str_ireplace("%IMGTHUMB%", trim($message['imageURL']['thumb'], $msg));
        if (preg_match('%IMGORIGINAL%', $msg)) $msg = str_ireplace("%IMGORIGINAL%", trim($message['imageURL']['original'], $msg));
        ...
        if (preg_match('%ANNOUNCE%', $msg)) {
            $sText = trim($message['announce']) != '' ? $message['announce'] : nsTrnc($message['text'], 300, " ", "...");
        ...
        if (preg_match('%TAGS%', $msg)) {
            if (!empty($message['tags'])) $tags = nxs_doProcessTags($message['tags']); else $tags['tags'] = '';
            $msg = str_ireplace("%TAGS%", $tags['tags'], $msg);
        }
        if (preg_match('%HTAGS%', $msg)) {
            if (!empty($message['tags'])) $tags = nxs_doProcessTags($message['tags']); else $tags['htags'] = '';
            $msg = str_ireplace("%HTAGS%", $tags['htags'], $msg);
        }
        if (preg_match('%CATS%', $msg)) {
            if (!empty($message['cats'])) $tags = nxs_doProcessTags($message['cats']); else $tags['cats'] = '';
            $msg = str_ireplace("%CATS%", $tags['tags'], $msg);
        }
        if (preg_match('%HCATS%', $msg)) {
            if (!empty($message['cats'])) $tags = nxs_doProcessTags($message['cats']); else $tags['hcats'] = '';
            $msg = str_ireplace("%HCATS%", $tags['htags'], $msg);
        }
...
if (!function_exists('nxs_filterOutSettings')) {
    function nxs_filterOutSettings($ntArr, $allOptions)
    {
        $ntArrOut = array();
        $arrNts = array_keys($ntArr);
...
if (!function_exists("nxs_chArrVar")) {
    function nxs_chArrVar($arr, $varN, $varV)
    {
        return (isset($arr) && is_array($arr) && isset($arr[$varN]) && $arr[$varN] == $varV);
    }
}
if (!function_exists('nxsMergeArraysOV')) {
    function nxsMergeArraysOV($Arr1, $Arr2)
    {
        foreach ($Arr2 as $key => $value) {
            if (array_key_exists($key, $Arr1) && is_array($value)) $Arr1[$key] = nxsMergeArraysOV($Arr1[$key], $Arr2[$key]); else $Arr1[$key] = $value;
        }
        return $Arr1;
    }
}
if (!function_exists('nxs_MergeCookieArr')) {
    function nxs_MergeCookieArr($ArrO, $ArrN)
    {
        $namesArr = array();
        foreach ($ArrO as $key => $value) {
            if (is_object($value)) $namesArr[$key] = $value->name;
        }
        foreach ($ArrN as $key => $value) {
            if (is_object($value) && $value->value != 'deleted') {
                $isEx = array_search($value->name, $namesArr);
                if ($isEx === false) $ArrO[] = $value; else $ArrO[$isEx] = $value;
            }
        }
        return $ArrO;
    }
}
if (!function_exists('nxs_doProcessTags')) {
    function nxs_doProcessTags($tags)
    {
        $tagsA = array();
        if (!is_array($tags)) {
            $tags = explode(',', $tags);
            foreach ($tags as $tg) $tagsA[] = trim($tg);
        } else $tagsA = $tags;
        $tagsA = array_unique($tagsA);
        $tags = array();
        foreach ($tagsA as $tg) {
            $tags['tagsA'][] = $tg;
            $tags['htagsA'][] = "#" . trim(str_replace(' ', '', preg_replace('/[^a-zA-Z0-9\p{L}\p{N}\s]/u', '', trim(ucwords(str_ireplace('&', '', str_ireplace('&amp;', '', $tg)))))));
        }
        $tags['tags'] = implode(', ', $tags['tagsA']);
        $tags['htags'] = implode(', ', $tags['htagsA']);
        return $tags;
    }
}
if (!function_exists('nxs_showListRow')) {
    function nxs_showListRow($ntParams)
    {
        $ntInfo = $ntParams['ntInfo'];
        $nxs_plurl = $ntParams['nxs_plurl'];
        $ntOpts = $ntParams['ntOpts'];  ?>
        <div class="nxs_box">
            ...
            <div class="nxs_box_inside">
                <?php if (!empty($ntParams['checkFunc']) && !function_exists($ntParams['checkFunc']['funcName'])) echo $ntParams['checkFunc']['msg']; else foreach ($ntOpts as $indx => $pbo) {
                    if (trim($pbo['nName'] == '')) $pbo['nName'] = $ntInfo['name'];
                    if (empty($pbo[$ntInfo['lcode'] . 'OK'])) $pbo[$ntInfo['lcode'] . 'OK'] = !empty($pbo[$ntParams['chkField']]) ? '1' : ''; ?>
                    <p style="margin:0px;margin-left:5px;"><img id="<?php echo $ntInfo['code'] . $indx; ?>LoadingImg"
                                                                style="display: none;"
                    ...
                    <div id="nxsNTSetDiv<?php echo $ntInfo['code'] . $indx; ?>"></div>
                <?php } ?>
            </div>
        </div> <?php
    }
...
if (!function_exists('nxs_doShowHint')) {
    ...
    {
        ?>
        <div id="<?php echo $t; ?>Hint" class="nxs_FRMTHint"
             style="font-size: 11px; margin: 2px; margin-top: 0px; padding:7px; border: 1px solid #C0C0C0; width: <?php echo $wdth; ?>%; background: #fff; display: none;">
            <span class="nxs_hili">%TITLE%</span> - <?php _e('Inserts the Title of the post', 'nxs_snap'); ?>, <span
                class="nxs_hili">%URL%</span> - <?php _e('Inserts the URL of the post', 'nxs_snap'); ?>, <span
                class="nxs_hili">%SURL%</span>
            - <?php _e('Inserts the <b>shortened URL</b> of your post', 'nxs_snap'); ?>, <span
                class="nxs_hili">%IMG%</span> - <?php _e('Inserts the featured image URL', 'nxs_snap'); ?>, <span
                class="nxs_hili">%EXCERPT%</span>
            - <?php _e('Inserts the excerpt of the post (processed)', 'nxs_snap'); ?>, <span class="nxs_hili">%RAWEXCERPT%</span>
            - <?php _e('Inserts the excerpt of the post (as typed)', 'nxs_snap'); ?>, <span
                class="nxs_hili">%ANNOUNCE%</span>
            - <?php _e('Inserts the text till the &lt;!--more--&gt; tag or first N words of the post', 'nxs_snap'); ?>,
            <span class="nxs_hili">%FULLTEXT%</span>
            - <?php _e('Inserts the processed body(text) of the post', 'nxs_snap'); ?>, <span
                class="nxs_hili">%RAWTEXT%</span>
            - <?php _e('Inserts the body(text) of the post as typed', 'nxs_snap'); ?>, <span
                class="nxs_hili">%TAGS%</span> - <?php _e('Inserts post tags', 'nxs_snap'); ?>, <span class="nxs_hili">%CATS%</span>
            - <?php _e('Inserts post categories', 'nxs_snap'); ?>, <span class="nxs_hili">%HTAGS%</span>
            - <?php _e('Inserts post tags as hashtags', 'nxs_snap'); ?>, <span class="nxs_hili">%HCATS%</span>
            - <?php _e('Inserts post categories as hashtags', 'nxs_snap'); ?>, <span
                class="nxs_hili">%AUTHORNAME%</span> - <?php _e('Inserts the author\'s name', 'nxs_snap'); ?>, <span
                class="nxs_hili">%SITENAME%</span> - <?php _e('Inserts the the Blog/Site name', 'nxs_snap'); ?>
            . <?php echo $ex; ?></div>
    <?php
    }
}
if (!function_exists('nxs_adjRpst')) {
...
if (!function_exists('nxs_replNRBack')) {
    function nxs_replNRBack(&$v, $k)
    {
        if (is_string($v)) {
            $v = str_replace("҈", "\r", $v);
            $v = str_replace("҉", "\n", $v);
...
if (!function_exists('nxs_maybe_unserialize')) {
    function nxs_maybe_unserialize($original)
    {
        if (is_serialized($original)) {
            $__ret = $original;
            $retArr = @unserialize($__ret);
            if ($retArr === false) {
                $__ret = str_replace("\r", "҈", $original);
                $__ret = str_replace("\n", "҉", $__ret);
                $retArr = @unserialize($__ret);
            }
            if (is_array($retArr)) {
                array_walk_recursive($retArr, 'nxs_replNRBack');
                return $retArr;
            } else return "Incorrect configuration data";
...
class NXS_HtmlFixer
{
    ...
    private function fixTag($t)
    {
        ...
        preg_match_all('/(?:"[^"]*"|\'[^\']*\'|[^"\'\s]+)+/', $t, $ar);
        $ar = $ar[0];// prr($ar);
        $nt = "";
        for ($i = 0; $i < count($ar); $i++) {
            if (strpos($ar[$i], 'href=\\\\\\"') !== false) {
    ...
    private function mergeStyleAttributes($s)
    {
        ...
        if (count($this->allowed_styles) > 0) {
            ...
            foreach ($check as $chk) {
                foreach ($this->allowed_styles as $as) if (stripos($chk, $as) !== False) {
                    $x .= $chk . ';';
                    break;
                }
            }
            ...
            if ($this->matrix[$i]["parentTag"] == $parentTag) {
                ...
                if ($this->matrix[$i]["tag"] != "") {
                    ...
                    if ($this->matrix[$i]["tagType"] != "") {
                        if (!in_array($this->matrix[$i]["tagType"], array("br", "img", "hr", "input"))) $out .= "</" . $this->matrix[$i]["tagType"] . ">";
                    }
                }
            }
        }
    ...
    private function charByCharJob()
    {
        ...
        while ($i < strlen($s)) {
            if ($s[$i] == "<") {
                ...
                $tag .= $s[$i];
                if (stristr($tag, '<param') && stristr($tag, '/>')) $tag = str_replace('/>', '></param>', $tag);
                $ss .= $tag;
            } else $ss .= $s[$i];
            $i++;
        }
        ...
        while ($i < strlen($s)) {
            if ($s[$i] == "<") {
                ...
                if ($s[$i] == ">") {
                    ...
                    $tagok = true;
                    if (($tags[$tagType] == 0) && (stristr($tag, '/' . $tagType . '>')) && (stristr($tag, '<' . $tagType) !== false)) {
                        $tagok = false;
        ...
        $errorsCounter = $this->checkTree();
        $this->fixedxhtml = $this->findSonsOf(0);
        return $errorsCounter;
    }
}
