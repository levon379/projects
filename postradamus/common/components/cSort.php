...
class cSort extends Object
{
    ...
    public function init()
    {
        if(!$this->key)
        {
            $this->key = 'sort' . md5(Yii::$app->request->get('r')) . Yii::$app->user->id;
        }
        if(Yii::$app->request->get('sort'))
        {
            Yii::$app->cache->set($this->key, Yii::$app->request->get('sort'));
        }
    ...
    public function getOrders($recalculate = false)
    {
        ...
        foreach ($attributeOrders as $attribute => $direction) {
            if($this->hasAttribute($attribute)) //nate modification
            {
    ...
    public function getAttributeOrders($recalculate = false)
    {
        if ($this->_attributeOrders === null || $recalculate) {
            ...
            if (empty($this->_attributeOrders) && is_array($this->defaultOrder) && Yii::$app->cache->get($this->key) == '') {
                $this->_attributeOrders = $this->defaultOrder;
            }
            if (empty($this->_attributeOrders))
            {
                if(Yii::$app->cache->get($this->key) != '' && Yii::$app->request->get('sort') == '')
                {
                    //if minus is first char then make it sort_desc and remove the minus
                    if(strstr(Yii::$app->cache->get($this->key), '-'))
                    {
                        $sort_name = str_replace('-', '', Yii::$app->cache->get($this->key));
                        $sort_direction = SORT_DESC;
                    }
                    else
                    {
                        $sort_name = Yii::$app->cache->get($this->key);
                ...
                $this->_attributeOrders = $default_sort;
            }
        }
