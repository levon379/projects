...
class rates_model extends CI_Model
{
	...
	function get_all_rates(){
		...
		$array_rs=$this->db->get_where("lb_bunglow", array("type"=>"B","is_active"=>"Y"))->result_array();
			
			//echo "<pre>"; print_r($array_rs); die;
			
		$bunglow_arr=array();//An array to store values
		$i=0;
		foreach($array_rs as $bunglow_id){
			$bunglow_main=$this->db->get_where("lb_bunglow", array("id"=>$bunglow_id['id']))->result_array();
			//Getting Bunglow details according to language and store name of the bunglow in array
			$bunglow_details=$this->db->get_where("lb_bunglow_lang", array("bunglow_id"=>$bunglow_id['id'], "language_id"=>$language_id))->result_array();
			$bunglow_arr[$i]['bunglow_id']=$bunglow_details[0]['bunglow_id'];
			$bunglow_arr[$i]['bunglow_name']=$bunglow_details[0]['bunglow_name'];
			$bunglow_arr[$i]['sort_order']=$bunglow_id['sort_order'];
			...
			foreach($season_arr as $season){
				...
				$seasons_rate_arr=$this->db->get_where("lb_bunglow_rates", array("bunglow_id"=>$bunglow_id['id'], "season_id"=>$season['id']))->result_array();
				if($seasons_rate_arr){
...
*/
		/*
		usort($bunglow_arr, function($a, $b) {
			return $a['bunglow_name'] - $b['bunglow_name'];
		});
		...
		usort($bunglow_arr, "method1");*/
		/*
		$usernames = array(); 
		foreach ($bunglow_arr as $b_arr) {
			$usernames[] = $b_arr['sort_order'];
		}
		array_multisort($usernames, SORT_ASC, $bunglow_arr);
		*/
		//echo "<pre>"; print_r($bunglow_arr); die;
