...
class home_model extends CI_Model
{
	...
	function get_unique_years_from_reservation($year)
	{
		$reservation=$this->db->get("lb_reservation")->result_array();
		$all_years_arr=array();
		if(count($reservation)==0)
		{
			$all_years_arr[0]=$year;
		}
		else 
		{
			...
			$arrival_years_array=$this->db->query("SELECT distinct YEAR(`arrival_date`) as distinct_year from `lb_reservation` WHERE YEAR(`arrival_date`)='".$curr_year."'")->result_array();
			$leave_years_array=$this->db->query("SELECT distinct YEAR(`leave_date`) as distinct_year from `lb_reservation` WHERE YEAR(`leave_date`)='".$curr_year."'")->result_array();
			
			//print_r($leave_years_array);die;
		...
		//print_r($all_years_arr);die;
		//sort($all_years_arr);
		$return_arr = array_unique($all_years_arr);
		//print_r($return_arr);
		return $return_arr;
	}
	...
	/*function getYears()
	{
		...
		array_push($all_years_arr, $curr_year);
		array_push($all_years_arr, $next_year);
		//print_r($all_years_arr);
		return $all_years_arr;
	}
	
	function getArrivalFromReservation($all_years_arr)
	{
		$first =  $all_years_arr[0].'-01-01';
		$last =   $all_years_arr[1].'-12-01';
		//echo $first;
		$query = $this->db->query("SELECT `arrival_date` FROM `lb_reservation` WHERE `arrival_date` BETWEEN '".$first."' AND '".$last."'");
		$result = $query->result_array();
		
		return $result;
	}*/
	
	/*function get_years_with_months()
	{
		...
		//echo '<br/>';
		$totalMonthDisplay=$currentMonth+2;
		//die;
		$i=0;
		//$n=date('m');
		...
		{
			for ($m=currentMonth; $m<=$totalMonthDisplay; $m++) 
			{
				$total_days=cal_days_in_month(CAL_GREGORIAN, ($m+1), $currentYear);
				$month_name= date('F', mktime(0,0,0,($m+1), 1, $currentYear));
				$all_months_arr[$m]['month']=($m+1);
				$all_months_arr[$m]['month_name']=$month_name;
				$all_months_arr[$m]['total_days']=$total_days;
				$all_months_arr[$m]['years']=$currentYear;
			}
	...
	function get_years_with_months($unique_years,$currentMonth,$currentYear)
	{
		$all_months_arr=array();
		$particular_year=array();
		$i=$currentMonth;
		//$n=date('m');
		//print_r($unique_years);die;
		//$curr_year = date("Y");
		//echo $currentMonth.'@@@'.$currentYear.'<br />';die;
		foreach($unique_years as $year)
		{
			
			for ($m=$currentMonth-1;$m<=($currentMonth+1);$m++) 
			{
				...
				$total_days=cal_days_in_month(CAL_GREGORIAN, ($y), $years);
				$month_name= date('F', mktime(0,0,0,($x+1), 1, $years));
				$all_months_arr[$x]['month']=$x;
				$all_months_arr[$x]['month_name']=$month_name;
				$all_months_arr[$x]['total_days']=$total_days;
				$all_months_arr[$x]['years']=$years;
			}
			array_push($particular_year, $all_months_arr);
			$i++;
		}
	...
	function get_bungalows()
	{
		...
		if(count($bungalow_arr)>0)
		{
			foreach($bungalow_arr as $bungalows)
			{
				...
				$reservation_array=$this->db->get_where("lb_reservation", array("bunglow_id"=>$bungalow_id, "is_active !="=>'DEACTIVE'))->result_array();
	...
	function get_latest_booking()
	{
		...
		foreach($reservation_list as $value)
		{
			...
			if($hour<24)
			{
				...
				$latest_reservation_arr['source']=$reservation_details[0]['source'];
			}
