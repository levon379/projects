...
class reservation_model extends CI_Model
{
	...
	function get_bungalows_with_rate($bungalow_id)
	{
		...
		$arrival_date=strtotime($reservation_session['arrival_date']);
		$leave_date=strtotime($reservation_session['leave_date']);
		...
		if(!empty($reservation_session['options_id']))
		{
			...
			foreach($options_arr as $options_id)
			{
				...
				if($currency_arr[0]['currency_id']==1)
				{
					...
					$total_options_rate_arr[$x]['options_charges']=$options_details[0]['charge_in_dollars']*$diff;
					$total_options_rate=$total_options_rate+($options_details[0]['charge_in_dollars']*$diff);
				}
				elseif($currency_arr[0]['currency_id']==2)
				{
					$total_options_rate_arr[$x]['options_name']=$options_details[0]['options_name'];
