...
class send_email_to_users_model extends CI_Model
{
	...
	function send_email()
	{
		...
		$posted_template_id=$this->input->post("template_id");
                $sendemailto=$this->input->post("sendemailto");
...
/* 				echo "asdasd";echo "<pre>";
print_r($this->input->post());echo "asdasd";die; */
			...
			foreach($languages_err as $language_det)
			{
				if($user_language_id==$language_det['id'])
				{
					...
					<tr>
						...
						<p><b>Les Balcons d\'Oyster Pond</b></p>
						</td>
					</tr>
					</table>';
					...
					//$this->email->from('info@les-balcons.com', 'LES BALCONS');
                                       $this->email->from('lesbalcons@lesbalcons.com', 'LES BALCONS');
					$this->email->to($to2); 
		...
		if(count($parent_info)>0) //If invoice is not available
		{
			...
			if(count($results_array1) > 0){
				...
				/* $msg_text2 =  $this->send_invoice($res_id);	
							echo "asdddd";var_dump($msg_text2);die;
