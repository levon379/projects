...
class bunglow_model extends CI_Model
{
	...
	function get_bunglow_testimonials($bunglow_id)
	{
		$this->db->order_by("id", "desc");
		$result=$this->db->get_where("lb_testimonials", array("bunglow_id"=>$bunglow_id))->result_array();
		return $result;
	}

	function delete_testimonials($testimonial_id)
	{
		$result=$this->db->delete("lb_testimonials", array("id"=>$testimonial_id));
	}
	
	function testimonial_status_change($test_id, $status)
	{
		$result=$this->db->update("lb_testimonials", array("status"=>$status), array("id"=>$test_id));
		return $result;
	}
	...
	function get_properties_rates($slug)
	{
		...
		foreach($season_arr as $season)
		{
			...
			if($seasons_rate_arr)
			{
				...
				$rates_arr[$j]['discount']=$seasons_rate_arr[0]['discount'];
			}
			else 
			{
				$rates_arr[$j]['season_id']=$season['id'];
	...
	function get_testimonials($slug)
	{
		$get_main_data=$this->db->get_where("lb_bunglow", array("slug"=>$slug))->result_array();
		$auto_id=$get_main_data[0]['id'];
		$this->db->order_by("id", "desc");
		$result=$this->db->get_where("lb_testimonials", array("bunglow_id"=>$auto_id, "status"=>"APPROVED"))->result_array();
		return $result;
	}
	...
	function get_bungalows($limit, $offset)
	{
		...
		$bungalows_arr=array();
		$this->db->order_by("id", "desc");
		$array=$this->db->get_where("lb_bunglow", array("is_active"=>"Y", "type"=>"B"), $limit, $offset)->result_array();
		$i=0;
		foreach($array as $value)
		{
