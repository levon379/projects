...
*/
		...
		$rate_details_arr=$this->db->get_where("lb_bunglow_rates", array("bunglow_id"=>$bungalow_id, "season_id"=>$season_id))->result_array();
		if( $accommodation > 6 ){
			$stay_euro = $accommodation * $rate_details_arr[0]['extranight_perday_europrice'];
		}else {
			$stay_euro = $rate_details_arr[0]['rate_per_day_euro'] * $accommodation;
		}
		...
		if($default_language_id==1)//If user language is english
		{
			...
			$msg_text2.='
				...
				<tr bgcolor="#f5e8c8">
					<td style="border-top:1px solid #C9AD64; padding:5px;"><b>Total Amount:</b></td><td style="border-top:1px solid #C9AD64; padding:5px; border-left:1px solid #C9AD64;">'.$default_currency.' '.$totalAmt.'</td>
				</tr>
				<tr>
				...
				<tr bgcolor="#f5e8c8">
					<td style="border-top:1px solid #C9AD64; padding:5px;"><b>Due Amount:</b></td><td style="border-top:1px solid #C9AD64; padding:5px; border-left:1px solid #C9AD64;">'.$default_currency.' '.$dueAmt.'</td>
				</tr>								
				<tr>
...
<br/>
	...
	function send_invoice_multi($reservation_id)
	{
		...
		$rate_details_arr=$this->db->get_where("lb_bunglow_rates", array("bunglow_id"=>$bungalow_id, "season_id"=>$season_id))->result_array();
		if( $accommodation > 6 ){
			$bungalow_rate = $accommodation * $rate_details_arr[0]['extranight_perday_europrice'];
		}else {
			$bungalow_rate = $rate_details_arr[0]['rate_per_day_euro'] * $accommodation;
		}
		//$bungalow_rate=ceil($reservation_details[0]['amount_to_be_paid'] - ($reservation_details[0]['amount_to_be_paid'] * 4/100));		
		...
		***/
		...
		if($reservation_details[0]['discount']!=0 && $reservation_details[0]['discount']!='')
		$discount1 = ($tot * $reservation_details[0]['discount']/100);
		else
		$discount1=0;
		...
		if($default_language_id==1)//If user language is french
		{
			...
			$msg_text2.='
				...
				if($reservation_details[0]["reservation_status"]!="Annulée")
				{
					...
					<tr>
						<td colspan="10"><b>Extra Person Amount: </b>'.$default_currency.' '.$extra_person.', <b>Tax Amount: </b>'.$default_currency.' '.($tax).', <b>Total Amount: </b>'.$default_currency.' '.$totalAmt.','.$discountStr.' <b>Paid Amount: </b>'.$default_currency.' '.$reservation_details[0]['paid_amount'].', <b>Due Amount: </b>'.$default_currency.' '.$dueAmt.'</td>
					</tr>';
				}
				else
				{
			...
			if(count($parent_info)>0) //If invoice is not available
			{
				foreach ($parent_info as $parent_details) 
				{
					...
					$prate_details_arr=$this->db->get_where("lb_bunglow_rates", array("bunglow_id"=>$parent_details["bunglow_id"], "season_id"=>$pseason_id))->result_array();
					//$parent_bungalow_rate = $paccommodation * $prate_details_arr[0]['extranight_perday_europrice'];


					if( $paccommodation > 6 ){
						$parent_bungalow_rate = $paccommodation * $prate_details_arr[0]['extranight_perday_europrice'];
					}else {
						$parent_bungalow_rate = $prate_details_arr[0]['rate_per_day_euro'] * $paccommodation;
					}
					...
					<tr>
						<td colspan="10"><b>Extra Person Amount: </b>'.$default_currency.' '.$pextra_person.', <b>Tax Amount: </b>'.$default_currency.' '.($ptot * 4/100).', <b>Total Amount: </b>'.$default_currency.' '.$totalAmt.','.$discountStr.' <b>Paid Amount: </b>'.$default_currency.' '.$parent_details['paid_amount'].', <b>Due Amount: </b>'.$default_currency.' '.$dueAmt.'</td>
					</tr>';
			...
			if(count($parent_info)>0) //If invoice is not available
			{
				foreach ($parent_info as $parent_details) {
					...
					<tr>
						<td colspan="10"><b>Prix personnes sup: </b>'.$default_currency.' '.$pextra_person.', <b>Taxe: </b>'.$default_currency.' '.($ptot * 4/100).', <b>Total du séjour: </b>'.$default_currency.' '.$totalAmt.','.$discountStr.' <b>Acompte payé: </b>'.$default_currency.' '.$parent_details['paid_amount'].', <b>Montant dû: </b>'.$default_currency.' '.$dueAmt.'</td>
					</tr>';
					}
				}
			}
