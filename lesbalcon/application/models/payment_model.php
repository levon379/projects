...
class payment_model extends CI_Model
{
	...
	function get_payment($type="")
	{
		...
		foreach($result as $row)
		{
			...
			if(count($parent_result)) {
				...
				foreach($parent_result as $prow)
				{
					if($b_id != $prow['bunglow_id']) {
						...
						$amount_to_be_paid = (float)($amount_to_be_paid + $prow["amount_to_be_paid"]);
					}
				}
			}
			...
			$created_by= ucfirst($admin_arr[0]['username']);
                        ...
                        $totalamountpaid=$totalamountpaid + $amount_to_be_paid;
			...
			$i++;
                ...
                // add
                        $payment_arr[$i]['id']='';
			$payment_arr[$i]['name']='';
			$payment_arr[$i]['paid_amount']=$default_currency.$totalamountpaid;
			$payment_arr[$i]['payment_mode']='';
			$payment_arr[$i]['reservation_date']='';
			$payment_arr[$i]['bunglow_name']='';
			$payment_arr[$i]['arrival_date']='';
			$payment_arr[$i]['leave_date']='';
			$payment_arr[$i]['reservation_status']='';
			$payment_arr[$i]['payment_status']='';
			$payment_arr[$i]['parent_bunglow']='';
			//$payment_arr[$i]['is_active']=$row['is_active'];
			$payment_arr[$i]['source']='';
			$payment_arr[$i]['created_by']='';
	...
	function ajax_edit_amount($id, $txt_discount='',$txt_total='',$txt_paid_amount='',$txt_due_amount='',$admin_comments='',$invoice_comments='',$source='')
	{
		...
		$txt_paid_amount=str_replace(',', '.', $txt_paid_amount);
		$new_txt_paid_amount = explode('.',$txt_paid_amount); 
		if(count($new_txt_paid_amount) == 3 && (strlen($txt_paid_amount) == 8 || strlen($txt_paid_amount) == 7)){
			$txt_paid_amount = $new_txt_paid_amount[0].$new_txt_paid_amount[1].'.'.$new_txt_paid_amount[2];
		}
		//$txt_paid_amount = $new_txt_paid_amount[0].$new_txt_paid_amount[1].'.'.$new_txt_paid_amount[2];
		$txt_due_amount=str_replace(',', '.', $txt_due_amount);
	...
	function ajax_change_amount($id, $paid,$due,$pay, $payment_mode='',$date_payment_mode='',$admin_comments='',$invoice_comments='')
	{
		$paid=str_replace(',', '.', $paid);
		$due=str_replace(',', '.', $due);
		$pay=str_replace(',', '.', $pay);
                
		//$paid = (float)($paid != "") ?$paid :"0";
		$due = (float)($due != "") ?$due :"0";
		$pay = (float)($pay != "") ?$pay :"0";
		$paid = $paid + $pay;
		$due = $due -$pay;
		...
		$this->db->update("lb_reservation", array("paid_amount"=>$paid, "due_amount"=>$due,"payment_mode"=>$payment_mode,"date_payment_mode" => $date_payment_mode, "admin_comments" => nl2br($admin_comments),"invoice_comments"=> $invoice_comments), array("id"=>$id));
	}
