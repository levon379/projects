...
class home_model extends CI_Model
{
	...
	function get_bungalows($arrival_date, $leave_date)
	{
		...
		if(count($bungalow_arr)>0)
		{
			foreach($bungalow_arr as $bungalows)
			{
				...
				$reservation_array=$this->db->get_where("lb_reservation", array("bunglow_id"=>$bungalow_id, "is_active !="=>'DEACTIVE', 'arrival_date >=' => $arrival_date, 'leave_date <=' => $leave_date ))->result_array();
	...
	function add_reservation()
	{
		...
		if($chk == "available"){
			...
			$val1 = ($this->input->post('no_of_adult') != "--Select--") ? $this->input->post('no_of_adult') :"0";
			$val2 = ($this->input->post('no_of_extra_real_adult') != "--Select--") ?$this->input->post('no_of_extra_real_adult') :"0";
			$val3 = ($this->input->post('no_of_extra_adult') != "--Select--") ?$this->input->post('no_of_extra_adult') :"0";
			$val4 = ($this->input->post('no_of_extra_kid') != "--Select--") ?$this->input->post('no_of_extra_kid') :"0";
			$val5 = ($this->input->post('no_of_folding_bed_kid') != "--Select--") ?$this->input->post('no_of_folding_bed_kid') :"0";
			$val6 = ($this->input->post('no_of_folding_bed_adult') != "--Select--") ?$this->input->post('no_of_extra_real_adult') :"0";
			$val7 = ($this->input->post('no_of_baby_bed') != "--Select--") ?$this->input->post('no_of_baby_bed') :"0";
	...
	function edit_reservation()
	{
		...
		$q_val = explode("/",$this->input->post("arrival_date"));	
		$season_id = $this->getSeasons($q_val[0],$q_val[1]);
...
echo $val1."-".$val2."-".$val3."-".$val4."-".$val5."-".$val6."-".$val7."-".$days_count;
	 	$total = ($val2 * 15 * $days_count) + ($val3 * 15 * $days_count) + ($val6 * 15 * $days_count);
		$final_amount = $total + $stay_euro + (($total + $stay_euro) * 4/100); 
...
print_r($upd_arr); die;
	...
	function getSeasons($day,$month){
		$cur_date = date('Y-m-d', mktime(0,0,0,$month,$day,date("Y")));
		$high_start_date = date('Y-m-d', mktime(0,0,0,12,15,date("Y")));
		$high_end_date = date('Y-m-d', mktime(0,0,0,4,14,date("Y")));

		$low_start_date = date('Y-m-d', mktime(0,0,0,4,15,date("Y")));
		$low_end_date = date('Y-m-d', mktime(0,0,0,12,14,date("Y")));

		if($cur_date > $high_start_date && $cur_date < $high_end_date) $season_id = "1";
		else if($cur_date > $low_start_date && $cur_date < $low_end_date) $season_id = "2";
