...
<div class="row innerpage-section">
	<div class="container">
		...
		<div class="row inner-content"><?php
		 
				if((count($search_result)>0) and !(isset($search_result['msg']))){
								...
								<div class="property-holder">
									...
									<div class="content-box">
										<div class="view-details"> <?php  
											if($value['type']=="P"){ ?>
												<a href="<?php echo base_url(); ?>properties/<?php echo $value['slug']; ?>"><?php echo lang('VIEW_DETAILS'); ?></a> <?php 
											}elseif($value['type']=="B"){
												if(isset($value['availability']) && $value['availability']!="available" && $value['availability']!="notavailable"){ ?>
													<a href="<?php echo base_url(); ?>bungalows/<?php echo $value['slug']; ?>?status=partial"><?php echo lang('VIEW_DETAILS'); ?></a> <?php
												}else{ ?>
													<a href="<?php echo base_url(); ?>bungalows/<?php echo $value['slug']; ?>"><?php echo lang('VIEW_DETAILS'); ?></a> <?php
												}
											} ?> 
										</div><?php 
										
										if(isset($value['availability']) && $value['availability']!="available" && $value['availability']!="notavailable"){ ?>
											<div class="book-now"><a style="cursor:pointer;" data-popup-target="#example-popup<?php echo $value['id'] ?>"><?php echo lang('BOOK_NOW'); ?></a></div><?php 
										} else { ?>
											<div class="book-now"><a href="<?php echo base_url(); ?>reservation/<?php echo $value['slug']; ?>"><?php echo lang('BOOK_NOW'); ?></a></div><?php 
										}  ?>
										
										<h5><?php  
											if(strlen(strip_tags($value['bunglow_name']))>70){
												echo substr(strip_tags($value['bunglow_name']), 0, 70)."...";  
											}else{ 
												echo strip_tags($value['bunglow_name']); 
											} ?>
										</h5>
										
										<p><?php
											$str = explode('<span>', $value['bunglow_name']);
											echo isset($str[1])?$str[1]:'';
											
											/*
											$overview_str = strip_tags($value['bunglow_overview']);
												echo substr($overview_str, 0, 80);  
											if(strlen($overview_str) > 80){
												echo "...";
											} */ ?>
										</p>
										
										<div class="persons"><?php
											echo $value['max_person'];
											if($value['max_person']==1){
												echo "&nbsp;".lang("PERSON")."&nbsp;".lang("PERSON1");
											}else{
												echo "&nbsp;".lang("PERSONS")."&nbsp;".lang("PERSONS1");
											} ?>
										</div>
										
										<div class="price"> <?php
										foreach ($all_rates as $details) {									
											if($value["id"] == $details["bunglow_id"]){
												echo $currency_symbol=$value['currency_symbol'];
												$currency_code=$value['currency_code'];

												echo "&nbsp;".$details['bunglow_price'];
											}
										}
										...
										if(isset($value['availability'])){
											if($value['availability']=="available"){ ?>
												<a><label><?php echo lang('AVAILABLE') ?></label></a><?php 
											}elseif($value['availability']=="notavailable"){ ?>
												<a><label><?php echo lang('NOT_AVAILABLE') ?></label></a><?php 
											}else{ ?>
												...
												<div id="example-popup<?php echo $value['id'] ?>" class="popup">
													<div class="popup-body">	
														...
														<div class="popup-content">
															...
															<label style="margin-top:5px;"><?php echo lang('available_on_date') ?> <?php echo lang('contact_info') ?></label>
														</div>
													</div>
												</div>
												<?php 
					...
					if(isset($search_result['msg']) and $search_result['msg']==2){ ?>
						<h2><?php echo lang('Minimum_Two_Days'); ?></h2><?php
					}elseif(isset($search_result['msg']) and $search_result['msg']==3){ ?>
						<h2><?php echo lang('Minimum_Three_Days'); ?></h2><?php
					}else{ ?>
						<h2><?php echo lang('No_records_found'); ?></h2><?php
					}
				}?>
			</div>
		</div>
