...
<script type="text/javascript">
	...
	$('.search_arrival_date').datetimepicker({
		format:'dd/mm/yyyy',
        language:  'Fr',
        weekStart: 1,
        todayBtn:  1,
...
$(function(){
	$('.time-line-cols td').on('contextmenu', function(e) {
		...
		if ($(e.target).prop('tagName')!=='TD') {
			$(e.target).hide();
		    unique_id=$(document.elementFromPoint(e.clientX, e.clientY)).closest('td').attr('id');
		    $(e.target).show();
		} else {
		...
		if(unique_id)
		{
			...
			if(reservation_id=="" && cleaning_date=="")// if date is not reserved and not reserved for cleaning
			{
				...
				$('#box').find('#print_details_id').hide();//Print details id will hide
			...
			else if(reservation_id!="")// if date is reserved
			{
				...
				$('#box').find('#print_details_id').show();//Print details id will show
				if(cleaning_date!="")
				{
					$('#box').find('#remove_cleaning_id').show();//Remove Cleaning will hide
					$('#box').find('#mark_cleaning_id').hide();
				}
				else{
					$('#box').find('#mark_cleaning_id').show();
					$('#box').find('#remove_cleaning_id').hide();
				}
			}
...
//coding for scrolling to current month
...
var element = document.getElementById(current_month_scroll);
if (element != null){
