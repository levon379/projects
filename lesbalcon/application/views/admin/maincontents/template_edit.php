	...
	<div class="row">
		<div class="col-md-12">
			...
			<div class="box box-primary">
				<form role="form" id="form" action="<?php echo base_url(); ?>admin/template/template_edit/<?php echo $this->uri->segment(4); ?>/<?php echo $this->uri->segment(5); ?>" method="POST" enctype="multipart/form-data" onsubmit="return check_form()">
					<div class="box-body">
						<div class="form-group">
							<label for="exampleInputEmail1">EMAIL TYPE<span style="color:red">*</span></label>
                                                        <select onchange="optionchangeAction()" name="mail_type" id="mail_type" class="form-control">
								<option value="">--Select--</option>
								<?php 
                                                                $show=false;
								foreach($email_type_arr as $email_type)
								{
                                                                    if($template_unique_details[0]['type_id']==7)
                                                                    $show=true;
									?>
									<option value="<?php echo $email_type['type_id'] ?>" <?php if($email_type['type_id']==$template_unique_details[0]['type_id']){ echo "selected";  } ?>><?php echo $email_type['title'] ?></option>
									<?php 
								}
								?>
							</select>
							<div class="form-group has-error" id="mail_type_error" style="display:none;">
								...
								<i class="fa fa-times-circle-o" > Email Type field is required</i>
                                            ...
                                            <div <?php echo ($show)?'':'style="display:none;" ';?>id="email_other_field" class="form-group">
									<label for="exampleInputPassword1">OTHER</label>
									<input type="text" name="email_other" id="email_other" class="form-control" placeholder="Write Other" value="<?php echo $template_arr[0]['other']?>">
						...
						<input type="hidden" value="<?php echo $this->uri->segment(5); ?>" name="template_id">
						<input type="hidden" value="<?php echo $this->uri->segment(4); ?>" name="language_id">
						<?php 
                                                
						$language_id_arr=array();
						foreach($template_arr as $language)
						{
							...
							<div id="div_<?php echo $language['id']; ?>">
								<div class="form-group">
									...
									<input type="text" name="email_subject<?php echo $language['id'] ?>" id="email_subject<?php echo $language['id'] ?>" class="form-control" placeholder="Subject" value="<?php echo $language['subject'] ?>">
								...
								<div class="form-group">
									...
									<?php								
										...
										$code = $CKEditor->editor("email_message".$language['id'], $language['message']);
										echo $code;
									?>
