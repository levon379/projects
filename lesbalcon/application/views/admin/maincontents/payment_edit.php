...
function validate_edit_reservation()
{
	...
	if($('#new_persons').hasClass("canValidate")){
                       ...
                       <input type="hidden" name="cur_bungalow_id" id="cur_bungalow_id" value="<?php echo $reservation_details[0]['bunglow_id']; ?>">
						...
						<div class="box-body">
							<div class="form-group">
                                ...
                                <!-- yrcode --->
                                <select onchange="change_no_of_adult(<?php echo $reservation_details[0]['bunglow_id']; ?>)" name="bungalow_id" id="bungalow_id" class="form-control">
								<?php 
...
<!-- end yrcode --->
							...
							<div id="saved_persons">
								<div class="form-group">
									...
									<a href="javascript:void(0)" onclick="var r = confirm('Etes-vous sûr de vouloir changer le nombre de personne');
																		  if (r == true) {
																		  	$('#saved_persons').hide(); $('#new_persons').show();$('#new_persons').addClass('canValidate'); $('#no_of_adult').prop('required','required'); 
																		  }else {
							...
							<div class="form-group" id="new_persons" style="display:none;">
								...
								<div class="form-group has-error" id="no_of_adult_error" style="display:none;">
									...
									<i class="fa fa-times-circle-o"> <?php echo lang('no_of_adult'); ?> is required</i>
                                                    ...
                                                    <script type="text/javascript">
                                                    function change_no_of_adult(selected_id){
                                                       var bungalowID=$('#bungalow_id').val();
                                                        if(selected_id==bungalowID){
                                                          $('#saved_persons').show(); $('#new_persons').hide();  $('#new_persons').removeClass('canValidate'); $('#no_of_adult').prop('required',false); 
                                                        }
                                                        else
                                                        {
                                                            $('#saved_persons').hide(); $('#new_persons').show(); $('#new_persons').addClass('canValidate'); $('#no_of_adult').prop('required','required');  
							...
							<div class="form-group">
								...
								<?php echo lang('Total_Amount'); ?>: <?php echo $totalAmountCalcul; ?><br/> 
								<?php
								$paidAmt = $reservation_details[0]["paid_amount"];
								...
								?>
								<?php echo lang('Paid_Amount'); ?>: <?php echo $paidAmt; //$reservation_details[0]["paid_amount"]; ?><br/>
								<?php echo lang('Due_Amount'); ?>: <?php echo $dueamount; //$reservation_details[0]["due_amount"]; ?><br/>
