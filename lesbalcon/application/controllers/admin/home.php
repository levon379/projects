...
class Home extends CI_Controller
{
	...
	function mobile()
	{
		...
		$unique_years = array(date('Y'),date('Y', strtotime('+1 year')));
		$this->data['latest_booking']=$this->home_model->get_latest_booking();
	...
	function ajax_get_reservation_details()
	{
		...
		<script type="text/javascript">
			function change_status(id, pstatus,rstatus)
			{
				...
				if(pay_amount){
					$.post("<?php echo base_url(); ?>admin/payment/ajax_change_amount", { "id":id, "paid_amount":paid_amount,"due_amount":due_amount,"pay_amount":pay_amount, "payment_mode":payment_mode,"date_payment_mode":date_payment_mode , "reservation_status" :$('#reservation_status').val(),"admin_comments":admin_comments,"invoice_comments":invoice_comments,"payment_status":$('#payment_status').val()}, function(data){});
				}else{
				...
				<div class="modal-header">
					...
					<table class="table table-hover">
						...
						<tr>
							...
							<!-- <td><?php echo $totalAmt;// $reservation_details["total"]; ?></td> -->
						...
						<tr>
							...
							<!-- <td><?php echo $paidAmt;// $reservation_details["paid_amount"]; ?></td> -->
							<td><input type="text" id="txt_paid_amount" onblur="checkDigitAfterDecimal('txt_paid_amount');" name="txt_paid_amount" value="<?php echo $paidAmt; ?>" /></td>
						</tr>								
						<tr>
						...
						<tr>
							...
							<td colspan="2">
								...
								<a class="btn btn-primary" href="<?php echo base_url(); ?>admin/users/invoice/<?php echo $reservation_id; ?>"><?php echo lang("send_mail"); ?></a>
		...
		if(count($parent_info)>0) //If invoice is not available
			...
			$results_array1=$this->db->get_where("lb_reservation", array("id"=>$results_array2[0]["parent_id"]))->result_array();
                        ...
                        if(count($results_array1) > 0){
                        	$msg_text2 =  $this->users_model->send_invoice_multi($results_array1[0]["id"]);
                                $msg_text2 .= '<style>table{ max-width:1050px; margin-left:-50px;}</style>';
			}
			else{
                            
				$msg_text2 =  $this->users_model->send_invoice($res_id);	
