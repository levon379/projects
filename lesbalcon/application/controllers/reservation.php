...
class reservation extends CI_Controller{
    ...
    function index($slug=""){
		...
		$this->data['content'] = $this->load->view('maincontents/reservation_short', $this->data, true);
	...
	function ajax_check_availability(){  
		...
		if($result=="available"){
			...
			echo json_encode(array('success'=>false, 'available'=>'no', 'price'=>false));
		}else{
	...
	function on_arrival_payment($payment_type)
	{
		...
		if($this->session->userdata("reservation"))	
		{
			...
			$ins_arr=array(
				...
				"payment_status"=>"En Attente", //Payment Status is En Attente because this is payment on arrival
				"is_active"=>"Activer",
				"color_code"=>$random_color,
				"source"=>'Website',
				"comments" =>nl2br($_POST['txt_comments']),
				"no_of_adult" => $reservation_session['no_of_adult'],
				"no_of_extra_adult" => $reservation_session['no_of_extra_adult'],
				"no_of_extra_real_adult" => $reservation_session['no_of_extra_real_adult'],
				"no_of_extra_kid" => $reservation_session['no_of_extra_kid'],
				"no_of_folding_bed_kid" => $reservation_session['no_of_folding_bed_kid'],
				"no_of_folding_bed_adult" => $reservation_session['no_of_folding_bed_adult'],
				"no_of_baby_bed" => $reservation_session['no_of_baby_bed']
			);

			$reservation_session['comments']=$_POST['txt_comments'];
			$this->session->set_userdata('reservation', $reservation_session);
	...
	function paypal_success()
	{
		...
		if($reservation_session['payment_type']=="Réglé")
		{
			$payment_type="Réglé";
			$payment_status="Annulé";
		}
		elseif($reservation_session['payment_type']=="Acompte Payé")
		{
			$payment_type="Acompte Payé";
			$payment_status="Annulé";
		}
