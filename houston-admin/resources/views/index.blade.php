...
@section('content')
<div class="page-header pg-tourmanager"><h1>Dashboard<small> Welcome</small></h1></div>
...
<div class="clearfix">
    <div class="row">
        <div class="col-md-12">
<div class="alert alert-success" role="alert"><h2>Hello {{ $user->firstname }} !</h2></div>

        </div>
    </div>
    <div class="alert alert-warning alert-dismissable">There are <a href="/admin/bookings/search?pr=1%2C2%2C3&bf=&bt=&br=&or=&tf=&tt=&pd=&po=&sg=&sn=&pm=&pa=&q=&pk=1&pb=1&pe=1"> {{ $pending_booking_count }} pending bookings  </a> that need your attention! </div>

    <div class="alert alert-info">Here's what's happening today:</div>

    <div class="row column-warpper" id="tour-manager-page">

        <!-- start of product -->
        @foreach($dates->dates as $index => $date)
            @if($startDate == date('d/m/Y',  strtotime($date->date)))
            <?php print_r($date->id)?>
            <div class=" tour-column {{ ((($index+1)%2)==0) ? 'even' : 'odd' }}" data-id = "{{$date->id}}" data-date="{{$date->date}}" data-total="{{ $date->total_bookings }}">
                    <p class="text-white margn-b-sm form-control-static">{{ App\Libraries\Helpers::displayShortDayDate($date->date) }} </p>
    ...
    @include('partials.welcome_slots')
    @stop
            ...
            $.get("/admin/services/services/type", null).done(function (data) {
                ...
                $(".tour-column").each(function () {
                    ...
                    $(document).bind('click', function (e) {
                        ...
                        if (!$clicked.parents().hasClass("guide-dropdown"))
                            $(".guide-dropdown dd ul").hide();
                    });
