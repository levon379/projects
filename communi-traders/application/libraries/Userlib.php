...
class Userlib {
       ...
       var $timestamp_delay   = 600;
        ...
        function Userlib()
        {
                ...
                $this->CI->db = $this->CI->load->database('default', TRUE, TRUE);
      ...
      /**
        ...
        function register( $username, $password, $email, $status = 1, $emailcheck = TRUE, $ipcheck = TRUE )
        {
                ...
                if( $emailcheck )
                {
                        ...
                        {
                                return 'Your e-mail already uses.';
                        }
                }
                ...
                {
                        return 'Your username already uses.';
                }

                $this->CI->db->query( "INSERT INTO `users` ( username, password, email, ip, status ) VALUES ( '"
                . mysql_real_escape_string( $username ) . "', '" . sha1( md5( $password ) ) . "', '"
                .mysql_real_escape_string( $email ) . "', '" . mysql_real_escape_string( $_SERVER[ 'REMOTE_ADDR' ] )
                ."', '" . mysql_real_escape_string( $status ) . "' )" );
                
                return 'New user added sucsessfully.';
        }
        ...
        {
                $qcheck = $this->CI->db->query( "SELECT * FROM `users` WHERE username='" . mysql_real_escape_string( $username ) . "' AND password='" . sha1( md5( $password ) ) . "' AND status <> 0" );
        ...
        function forgot( $email, $length )
        {
                ...
                if( $qemail->num_rows( ) > 0 )
                {
                        ...
                        mail( $email, "Password recowery system", "Your new password: " . $new );
