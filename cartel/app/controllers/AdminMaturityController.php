<?php
class AdminMaturityController extends \BaseController {
  ...
  public function __construct() {
    parent::__construct();
		$this->adminVars['adminWord']      = 'maturity';
		$this->adminVars['adminWords']     = 'maturitys';
		$this->adminVars['adminWordCap']   = 'Maturity';
		$this->adminVars['adminClassType'] = 'Maturity';
		$this->adminVars['adminURI']       = 'admin-maturity';
		$this->adminVars['adminActiveID']  = '19';
		$this->adminVars['adminDeleteID']  = '22';
	}	// __construct()
  ...
  *           Display a listing of the resource.
	...
	* @return Response
	...
	public function index() {
		/* Get the list of IDs */
		$itemIDs = Maturity::getItems($this->pageData['locale_id'],
                                  $this->pageData['language_id']);
		$items = array();
    
		/* Retrieve details for each item */
		foreach ($itemIDs as $value)
      $items[$value->id] = Maturity::getItemInfo($value->id);
	...
	* @return Response
    ...
    /* Is an existing item, so retrieve item details for the form */
		elseif(is_numeric($item_id)) {	
			$details = Maturity::getItemInfo($item_id);
      
      /* check to make sure the array isn't empty  */
			if (!count($details))  {	
        return Redirect::to($this->adminVars['adminURI'])
					->with('messages', 
              array(Lang::get('site_content_admin.global_admin_Item_Not_Exist'), $this->pageData['error']));
			}	
		}
  ...
  *             Save new or update the item.
	...
	* @return Response
    ...
    /* means a new item, so establish a new item class to save */		
		if($item_id == 0) {	
			$itemToSave = new Maturity;
		}
    /* Is an existing item, so establish item class to update */
		elseif (is_numeric($item_id)) {	
			$itemToSave = Maturity::find($item_id);
		}
	...
	* @return Response
	...
	public function destroy($item_id = 0) { 	
       ...
       status code assigned to it, to protect data integrety */
		$itemToDelete            = Maturity::findOrFail($item_id);
	...
	* @return Response
	...
	public function swap($item_id = 0, $swap_id = 0) { 	
		if($item_id == 0 || $swap_id == 0) {
			...
			$currItem = Maturity::findOrFail($item_id);
			$swapItem = Maturity::findOrFail($swap_id);
