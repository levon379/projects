		...
		<table class="table table-bordered table-striped">
			...
			<tbody>
				...
				<tr bgcolor='{{{$itemVar->status_color}}}'>
					<td>{{{$itemVar->created_at}}}</td>
					<td>
						<a href='/admin-company/{{{$itemVar->bidder_company_id}}}/edit/'>{{{$itemVar->bidder_company_name}}}</a><br />
						<a href='/admin-user/{{{$itemVar->bidder_id}}}/edit'>{{{$itemVar->bidder_name}}}</a>
					</td>
					<td>
						<a href='/admin-company/{{{$itemVar->product_owner_company_id}}}/edit/'>{{{$itemVar->product_owner_company_name}}}</a><br />
						<a href='/admin-user/{{{$itemVar->product_owner_id}}}/edit'>{{{$itemVar->product_owner_name}}}</a>
					</td>
					<td>
						{{{$itemVar->bid_status}}}<br />
						@if($itemVar->accounting_number)<a href='/admin-order/{{{$itemVar->order_id}}}/show'>{{{$itemVar->accounting_prefix.$itemVar->accounting_number}}}</a><br /> @endif
					</td>
				</tr>
				@endforeach
			</tbody> 
